...
bool Cbc::isTopSensor(ReadoutChip* pChip, uint16_t pLocalColumn)
{
    return (pLocalColumn %2 != 0) ? true : false;
}    
std::pair< uint16_t, uint16_t > Cbc::getGlobalCoordinates(ReadoutChip* pChip, uint16_t pLocalColumn, uint16_t pLocalRow)
{
    ...
    {
        cGlobalX = (pChip->getNumberOfCols()/2 - cCol) + (NCHIPS_OT - pChip->getId() -1)*pChip->getNumberOfCols()/2;
    }
    else
    {
