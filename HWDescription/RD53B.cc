...
float RD53B::VCal2Charge(float VCal, bool isNoise) const
{
    ...
    return (Vref / VrefDivider / RD53BchargeConvertion::ADCrange) * VCal / RD53BchargeConvertion::ele * (this->getRegItem("INJ_CAP").fValue * 1e3) +
...
float RD53B::Charge2VCal(float Charge) const
{
    ...
    const auto Vref        = this->getRegItem("VREF_ADC").fValue / 1000.; // @CONST@ : Conversion from [mV] to [V]
    const auto VrefDivider = (this->getRegItem("SEL_CAL_RANGE").fValue == 0 ? 2 : 1);
    return (Charge - RD53BchargeConvertion::offset) / (this->getRegItem("INJ_CAP").fValue * 1e3) * RD53BchargeConvertion::ele / (Vref / VrefDivider / RD53BchargeConvertion::ADCrange);
}
