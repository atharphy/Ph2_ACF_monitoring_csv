...
{
    constexpr ByteTransform(std::initializer_list<uint8_t> encoding) : size(encoding.size()), encoding_array{{0}}, decoding_array{{0}}
    {
        std::copy(encoding.begin(), encoding.end(), std::begin(encoding_array));
        for(size_t i = 0; i < size; ++i) decoding_array[encoding_array[i]] = i;
        for(size_t i = size; i < 256; ++i) decoding_array[encoding_array[i]] = size;
    }
...
constexpr ByteTransform FieldEncodingTransform({0b01101010, 0b01101100, 0b01110001, 0b01110010, 0b01110100, 0b10001011, 0b10001101, 0b10001110, 0b10010011, 0b10010101, 0b10010110,
...
constexpr ByteTransform TriggerPatternTransform(
    {0b00101011, 0b00101101, 0b00101110, 0b00110011, 0b00110101, 0b00110110, 0b00111001, 0b00111010, 0b00111100, 0b01001011, 0b01001101, 0b01001110, 0b01010011, 0b01010101, 0b01010110});

constexpr ByteTransform TriggerTagTransform({0b01101010, 0b01101100, 0b01110001, 0b01110010, 0b01110100, 0b10001011, 0b10001101, 0b10001110, 0b10010011, 0b10010101, 0b10010110,
                                             0b10011001, 0b10011010, 0b10011100, 0b10100011, 0b10100101, 0b10100110, 0b10101001, 0b01011001, 0b10101100, 0b10110001, 0b10110010,
                                             0b10110100, 0b11000011, 0b11000101, 0b11000110, 0b11001001, 0b11001010, 0b11001100, 0b11010001, 0b11010010, 0b11010100, 0b01100011,
                                             0b01011010, 0b01011100, 0b10101010, 0b01100101, 0b01101001, 0b00101011, 0b00101101, 0b00101110, 0b00110011, 0b00110101, 0b00110110,
                                             0b00111001, 0b00111010, 0b00111100, 0b01001011, 0b01001101, 0b01001110, 0b01010011, 0b01010101, 0b01010110, 0b01100110});
...
using LongCommand = NamedTuple<NamedField<"opCode", Constant<Uint<8>, OpCode>>,
                               NamedField<"fields", Compose3<ClassObject<Fields...>, Many<Uint<5>>, Many<Validated<Transformed<Uint<8>, FieldEncodingTransform>, [](const auto&, const auto& value) {
...
{
    using type = LongCommand<0b001011010, Field<&Clear::chip_id, Uint<5>>>;
};

template <>
struct command_encoding<GlobalPulse>
{
    using type = LongCommand<0b01011100, Field<&GlobalPulse::chip_id, Uint<5>>>;
};
...
{
    using type = LongCommand<0b01100110, Field<&WrRegLong::chip_id, Uint<5>>, Field<Discarded{}, Constant<Uint<10>, 0b1000000000>>, Field<&WrRegLong::values, Many<Uint<10>>>>;
};

template <>
struct command_encoding<RdReg>
{
    using type = LongCommand<0b01100101, Field<&RdReg::chip_id, Uint<5>>, Field<Discarded{}, Constant<Bool, 0>>, Field<&RdReg::address, Uint<9>>>;
};
...
{
    using type = ClassObject<Field<&Trigger::pattern, Transformed<Uint<8>, TriggerPatternTransform>>, Field<&Trigger::tag, Transformed<Uint<8>, TriggerTagTransform>>>;
};
