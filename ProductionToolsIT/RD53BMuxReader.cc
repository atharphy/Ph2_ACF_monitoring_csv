...
RD53BMuxReader::adc_result RD53BMuxReader::get_adc(Ph2_HwInterface::RD53Interface* chipInterface, Ph2_HwDescription::ReadoutChip* chip, const std::string& name, size_t nsample)
{
    ...
    if(n_valid > 0) { return {float(sum) / n_valid, sqrt(rms) / n_valid, line.str()}; }
    else { return {0, 0, ""}; }
}
...
void RD53BMuxReader::run()
{
    ...
    std::vector<std::string> tempsens_list = {
        "TEMPSENS_ANA_SLDO", "TEMPSENS_CENTER", "TEMPSENS_DIG_SLDO", "RADSENS_ANA_SLDO", "RADSENS_DIG_SLDO", "RADSENS_CENTER", "POLY_TEMPSENS_TOP", "POLY_TEMPSENS_BOTTOM", "INTERNAL_NTC_ABS"};
                ...
                for(const auto cChip: *cHybrid)
                {
                    ...
                    if(use_wlt_calibration)
                    {
                        ...
                        for(const auto& mux: muxlist)
                        {
                            ...
                            LOG(INFO) << BOLDBLUE << std::setw(24) << mux << " ADC = " << BOLDYELLOW << result.line << "  |   " << std::setw(7) << std::setprecision(1) << result.value << "   "
