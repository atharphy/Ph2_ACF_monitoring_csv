...
void TEPXQuadNTC::localConfigure(const std::string& histoFileName, int currentRun) {
  LOG(INFO) << GREEN << "[TEPXQuadNTC::localConfigure] Starting run: " << BOLDYELLOW << theCurrentRun << RESET;
}
...
void TEPXQuadNTC::run()
{
    ...
    for(const auto cBoard: *fDetectorContainer)
        for(const auto cOpticalGroup: *cBoard)
		    ...
		    if (saturation){
		      LOG(WARNING) << "failed to find a working range without saturation";
		    }else{
		      LOG(INFO) << "ntc dac step size set to " << dac_ntc_step
				<< "  max dac = " << num_dac_step * dac_ntc_step
				<< "  ref=" << max_r_ref_adc
				<< "  ntc = " << max_ntc_adc;
		    }
		      ...
		      if(fVerbose)
			{
			  ...
			  LOG(INFO) << "slope" << std::fixed << std::setw(8) << std::setprecision(2) << fit_ntc.first << " " << std::fixed << std::setw(8) << std::setprecision(2) << fit_ref.first << " "
				    << std::fixed << std::setw(12) << std::setprecision(3) << R_ntc_slope << std::fixed << std::setw(12) << std::setprecision(1) << TfromR(R_ntc_slope, R25NTC, beta);
