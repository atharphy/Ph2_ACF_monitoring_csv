    ...
    for(auto cBoard: *cTool.fDetectorContainer)
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if( cIter%10 == 0 )
                    LOG (INFO) << BOLDBLUE << "Npackets is " << +cNevents << " number of words in vector is " 
                        << cData.size() 
                        << " size of complete data is " <<  cCompleteData.size()
                        << RESET;
        ...
        for(auto& cEvent: cPh2Events)
        {
            ...
            if(cDAQFile || cDQM)
            {
                ...
                if(cDQM && cEventCounter % cScaleFactor == 0){ 
            ...
            { 
                LOG (INFO) << BOLDBLUE << "Event#" << +cEvent->GetEventCount()
                    << " trigger Id " << +cEvent->GetExternalTriggerId()
                    << RESET;
                // outp.str("");
                // outp << *cEvent;
