...
int main(int argc, char* argv[])
{
    ...
    std::pair<uint32_t, uint32_t> rows = {14, 16};
    std::pair<uint32_t, uint32_t> cols = {118, 120};

    std::pair<uint32_t, uint32_t> latrange =  {truelat-1, truelat};  
    std::pair<uint32_t, uint32_t> rx4range = {12, 13};
    std::pair<uint32_t, uint32_t> rx3range = {1, 2}; 
    std::pair<uint32_t, uint32_t> retimerange = {6, 8};
    ...
    for(size_t row = rows.first; row < rows.second; row++)
    {
		...
		for(auto cMPA: *ChipVec)
		{
			...
			if (cMPA->getFrontEndType() == FrontEndType::SSA)
			{
				...
				thePSInterface->WriteChipReg(cMPA, "DigCalibPattern_L_ALL", 0xFF);
				thePSInterface->WriteChipReg(cMPA, "DigCalibPattern_H_ALL", 0xFF);
			}
		...
		for(size_t ES = esrange.first; ES <esrange.second; ES++)
			...
			for(auto cMPA: *ChipVec)
			{
				...
				if (cMPA->getFrontEndType() == FrontEndType::MPA)
					...
					RX3towrite = (RX3towrite<<0x3)|1;//Hack, saves some looping
					//RX3towrite = RX3;
			...
			for(__attribute__((unused)) auto& ev: events) 
				...
				for(auto cMPA: *ChipVec)
				{
					...
					std::cout << "NPIX "<<NPclustot<< std::endl;
				        for( auto& pc: Pclus)  
				            {
					    if ((col+1)==pc.fAddress and (row)==pc.fZpos)MatchNPclustot+=1;
				             std::cout << "-------------------------------PIXELS-------------------------------"<< std::endl;
				             std::cout << "fAddress "<<+pc.fAddress<<" vs "<< col+1<< std::endl;
				             std::cout << "fWidth "<<+pc.fWidth<< std::endl;
				             std::cout << "fZpos "<<+pc.fZpos <<" vs "<< row<< std::endl;//Why row start from 0??
				            }
					std::cout << "NSTRIPS "<<NSclustot<< std::endl;
				        for( auto& sc: Sclus)  
				            {
					    if ((col+1)==sc.fAddress)MatchNSclustot+=1;
				             std::cout << "-------------------------------STRIPS-------------------------------"<< std::endl;
				             std::cout << "fAddress? "<<+sc.fAddress<<" vs "<< col+1<<std::endl;
				        ...
				        for( auto& st: stubs)  
				            {
				             ...
				             std::cout << "getPosition "<<+st.getPosition()<<" vs "<<(col+1)*2<< std::endl;
					     obs.push_back(st.getPosition());
					     exp.push_back(col);
					     if (st.getPosition()==((col+1)*2))MatchNstub+=1;
 			...
 			uint32_t totsum = MatchNstub+MatchNSclustot+MatchNPclustot;
  			if (NSclustot==MatchNSclustot)totsum+=MatchNSclustot;
  			if (NPclustot==MatchNPclustot)totsum+=MatchNPclustot;

			if ((MatchNSclustot>maxNSclustot) and (NSclustot==MatchNSclustot))
					{
					maxNSclustot=MatchNSclustot;
					}
			if (MatchNPclustot>maxNPclustot and (NPclustot==MatchNPclustot))
					{
					maxNPclustot=MatchNPclustot;
					}
			if (MatchNstub>maxNStubs)
					{
					maxNStubs=MatchNstub;
					}
			if (totsum>maxSUM)
					{
					maxSUM=totsum;
			...
			std::cout << "CURMAX "<<maxSUM<<" ilat "<<maxvals[0]<<" RX4 "<<maxvals[2]<<" RX3 "<<maxvals[3]<<" ES "<<maxvals[4]<<" RTR "<<maxvals[5]<<std::endl;
