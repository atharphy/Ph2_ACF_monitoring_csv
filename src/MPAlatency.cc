                ...
                for(auto cFe: *cOpticalGroup)
                {
                    ...
                    for(auto cChip: *cFe)
                    {
                        ...
                        if (cChip->getFrontEndType() == FrontEndType::SSA)
                            {
                            static_cast<SSAInterface*>(cTool.fReadoutChipInterface)->WriteChipReg(cChip, "ENFLAGS_ALL", 1,false);
                            }
                    }
    ...
    for(auto cOpticalGroup: *pBoard)
            {
                for(auto cFe: *cOpticalGroup)
                {
                    ...
                    for(auto cChip: *cFe)
                    {
                        if (cChip->getFrontEndType() == FrontEndType::MPA)
                            {
                            static_cast<MPAInterface*>(cTool.fReadoutChipInterface)->WriteChipReg(cChip, "ENFLAGS_ALL", 0,false);
                            }
                        if (cChip->getFrontEndType() == FrontEndType::SSA)
                            {
