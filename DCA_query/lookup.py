...
def fill_missing(entries, values, efuse_field, name, fallback):
    for efuse in sorted({entry[efuse_field] for entry in entries if entry[efuse_field] not in values}):
        value = fallback
        if not value and sys.stdin.isatty():
            value = input(f"No {name} found for eFuse {efuse}. Enter {name} (-1 to leave empty): ").strip()
        values[efuse] = "" if value == "-1" else value
...
def main():
    ...
    with args.output.open("w", newline="", encoding="utf-8") as stream:
        ...
        for entry in entries:
            entry["module_id"] = modules[entry["chip_efuse"]]
            entry["portcard_id"] = portcards[entry["lpgbt_efuse"]]
