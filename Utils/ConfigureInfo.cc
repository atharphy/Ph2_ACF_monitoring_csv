...
{
    if(fEnabledObjectStructure.find(boardId) == fEnabledObjectStructure.end())
    {
        fEnabledObjectStructure[boardId] = std::make_pair(boardName, OpticalGroupMap());
    }
    else fEnabledObjectStructure[boardId].first = boardName;
}
...
void ConfigureInfo::enableOpticalGroup(uint16_t boardId, uint16_t opticalGroupId,  const std::string opticalGroupName)
{
    ...
    if(opticalGroupMap.find(opticalGroupId) == opticalGroupMap.end())
    {
        opticalGroupMap[opticalGroupId] = std::make_pair(opticalGroupName, HybridMap());
    }
    else opticalGroupMap[opticalGroupId].first = opticalGroupName;
}
...
void ConfigureInfo::enableHybrid(uint16_t boardId, uint16_t opticalGroupId, uint16_t hybridId, const std::string hybridName)
{
    ...
    if(hybridMap.find(hybridId) == hybridMap.end())
    {
        hybridMap[hybridId] = std::make_pair(hybridName, ChipMap());
    }
    else hybridMap[hybridId].first = hybridName;
}
