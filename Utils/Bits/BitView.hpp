...
public:
    ...
    BitView slice(size_t start) const {
        if (start > size())
            throw std::runtime_error("BitView::slice: out of range.");
    ...
    void skip(size_t n) { 
        if (n > size())
            throw std::runtime_error("BitView::skip: out of range.");
        _start += n; 
    }
    ...
    T pop(size_t n) {
        if (n > size())
            throw std::runtime_error("BitView::pop: out of range.");
    ...
    BitView pop_slice(size_t n) {
        if (n > size())
            throw std::runtime_error("BitView::pop_slice: out of range.");
        size_t start = _start;
        _start += n;
