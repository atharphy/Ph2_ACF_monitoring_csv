#ifndef BITSERIALIZATION_TYPES_COMPOSE3_H
#define BITSERIALIZATION_TYPES_COMPOSE3_H
...
struct Compose3
{
    ...
    struct Error
    {
        ...
        friend std::ostream& operator<<(std::ostream& os, const Error& self) { return (os << "Compose3 " << stage_names[I] << " error: " << self.error); }
    };
    ...
    using ParseError = ErrorVariant<Error<0, parse_error_t<SourceType>>, Error<1, serialize_error_t<IntermediateType>>, Error<2, parse_error_t<DestinationType>>>;
    ...
    static ParseResult<value_type, ParseError> parse(const BitView<T>& bits, const U& parent = {})
    {
        ...
        if(!destination_result) return {make_error<2>(std::move(destination_result.error()))};
        ...
        if(!intermediate_result) return {make_error<1>(std::move(intermediate_result.error()))};

        // std::cout << "Compose3 parse buffer: " << new_bits.view() << std::endl;

        auto source_result = SourceType::parse(bit_view(new_bits), parent);
        if(!source_result) return {make_error<0>(std::move(source_result.error()))};

        return {std::move(source_result.value()), destination_result.size()};
    }

    using SerializeError = ErrorVariant<Error<0, serialize_error_t<SourceType>>, Error<1, parse_error_t<IntermediateType>>, Error<2, serialize_error_t<DestinationType>>>;

    template <class T, class U = Void>
    static SerializeResult<SerializeError> serialize(value_type& value, BitVector<T>& bits, const U& parent = {})
    {
        BitVector<uint64_t> new_bits;
