#ifndef BITSERIALIZATION_TYPES_OPTIONAL_H
#define BITSERIALIZATION_TYPES_OPTIONAL_H
...
struct Optional
{
    ...
    static ParseResult<value_type, parse_error_t<Type>> parse(const BitView<T>& bits, const U& parent)
    {
        if(Condition(parent))
        {
    ...
    static SerializeResult<serialize_error_t<Type>> serialize(value_type& value, BitVector<T>& bits, const U& parent = {})
    {
        if(Condition(parent))
        {
            auto v      = value.value();
