#ifndef BITSERIALIZATION_TYPES_MANY_H
#define BITSERIALIZATION_TYPES_MANY_H
...
struct Many
{
    ...
    struct ManySizeError
    {
        ...
        friend std::ostream& operator<<(std::ostream& os, const ManySizeError& error) { return (os << "Many error: not enough elements (" << error.size << " / " << MinSize << ")."); }
    };
    ...
    static ParseResult<value_type, ParseError> parse(const BitView<T>& bits, const U& parent = {})
    {
        ...
        while(offset < bits.size())
        {
            ...
            if(!result)
            {
                if(results.size() < MinSize) return {ManyElementError(results.size(), std::move(result.error()))};
                break;
            }
            results.push_back(std::move(result.value()));
            offset += result.size();
        }
        if(results.size() < MinSize) return {ManySizeError(results.size())};
