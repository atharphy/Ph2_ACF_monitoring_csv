#ifndef BITSERIALIZATION_TYPES_STRING_H
#define BITSERIALIZATION_TYPES_STRING_H
...
struct String
{
    ...
    static ParseResult<value_type, ParseError> parse(const BitView<T>& bits, const U& parent = {})
    {
        ...
        while(true)
        {
            ...
            if(terminator_result)
            {
                ...
                {
                    results.push_back(value_type_t<ElementType>(std::move(terminator_result.value())));
                    offset += terminator_result.size();
                }
                break;
            }
            auto element_result = ElementType::parse(bits.slice(offset), parent);
            if(!element_result) { return ParseError{results.size(), std::move(element_result.error())}; }
    ...
    using SerializeError = ErrorVariant<SerializeElementError, serialize_error_t<TerminatorType>>;
