#ifndef BITSERIALIZATION_TYPES_VALIDATED_H
#define BITSERIALIZATION_TYPES_VALIDATED_H
...
struct Validated
{
    ...
    using ParseError = ErrorVariant<_ValueError, parse_error_t<Type>>;
    ...
    using SerializeError = ErrorVariant<_ValueError, serialize_error_t<Type>>;

    template <class T, class U = Void>
    static SerializeResult<SerializeError> serialize(value_type& value, BitVector<T>& bits, const U& parent = {})
    {
        if(!Predicate(parent, value)) return {_ValueError{value}};
...
{
    using type = Validated<Type, [](const auto&, const auto& value) { return (value == Value); }>;
};
