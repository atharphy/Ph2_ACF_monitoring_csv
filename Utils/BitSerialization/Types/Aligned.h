...
struct Aligned
{
    ...
    static ParseResult<value_type_t<Type>, ParseError> parse(const BitView<T>& bits, const U& parent = {})
    {
        ...
        if(leftover_bits > 0)
        {
            ...
            if(bits.size() < result.size() + padding) return {_SizeError(bits.size())};
    ...
    struct SerializeSizeError
    {
        ...
        friend std::ostream& operator<<(std::ostream& os, const SerializeSizeError& self) { return (os << "Padded error: too many bits (" << self.size << '/' << Size << ")"); }
    };

    template <class T, class U = Void>
    static SerializeResult<serialize_error_t<Type>> serialize(value_type_t<Type>& value, BitVector<T>& bits, const U& parent = {})
    {
        ...
        if(leftover_bits > 0) bits.append_zeros(Size - leftover_bits);
        return {};
    }
};
