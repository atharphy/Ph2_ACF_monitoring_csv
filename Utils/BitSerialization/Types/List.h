#ifndef BITSERIALIZATION_TYPES_LIST_H
#define BITSERIALIZATION_TYPES_LIST_H
...
struct List
{
    ...
    static ParseResult<value_type, ElementError<parse_error_t<Type>, "List">> parse(const BitView<T>& bits, const U& parent = {})
    {
        ...
        for(size_t i = 0; i < size; ++i)
        {
            ...
            if(!parse_result) return {{results.size(), std::move(parse_result.error())}};
    ...
    struct SizeError
    {
        ...
        friend std::ostream& operator<<(std::ostream& os, const SizeError& error) { return (os << "List serialize error: not enough bits (" << error.size << " / " << error.required_size << ")."); }
    };
    ...
    {
        size_t size = SizeGetter(parent);
        if(size != value.size()) return {SizeError{size, value.size()}};
        for(size_t i = 0; i < value.size(); ++i)
        {
            auto result = Type::serialize(value[i], bits, parent);
            if(!result) return {SerializeElementError{i, std::move(result.error())}};
        }
        return {};
    }
};
