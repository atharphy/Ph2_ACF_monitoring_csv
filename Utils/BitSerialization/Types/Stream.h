#ifndef BITSERIALIZATION_TYPES_STREAM_H
#define BITSERIALIZATION_TYPES_STREAM_H
...
struct Stream
{
    ...
    using ParseError = ErrorVariant<InitiatorParseError, ElementError<parse_error_t<ElementType>, "Stream">>;
    ...
    static ParseResult<value_type, ParseError> parse(const BitView<T>& bits, const U& parent = {})
    {
        ...
        if(!initiator_result) return {InitiatorParseError{IncludeInitiator - 1, std::move(initiator_result.error())}};
        offset += initiator_result.size();
        if(IncludeInitiator) results.push_back(initiator_result.value());
        while(offset < bits.size())
        {
            auto element_result = ElementType::parse(bits.slice(offset), parent);
            if(!element_result) break;
    ...
    static SerializeResult<SerializeError> serialize(value_type& value, BitVector<T>& bits, const U& parent = {})
    {
        ...
        if(!initiator_result) return {InitiatorSerializeError{IncludeInitiator - 1, std::move(initiator_result.error())}};
        for(size_t i = IncludeInitiator ? 1 : 0; i < value.size(); ++i)
        {
            auto element_result = ElementType::serialize(value[i], bits, parent);
            if(!element_result) return {ElementSerializeError{i, std::move(element_result.error())}};
        }
        return {};
    }
};
