#ifndef BITSERIALIZATION_TYPES_SEPARATEDBY_H
#define BITSERIALIZATION_TYPES_SEPARATEDBY_H
...
struct SeparatedBy
{
    ...
    static ParseResult<value_type, ParseError> parse(const BitView<T>& bits, const U& parent = {})
    {
        ...
        while(true)
        {
            ...
            if(!element) return ParseError{results.size(), std::move(element.error())};
    ...
    using SerializeError = ErrorVariant<SerializeElementError, serialize_error_t<SeparatorType>>;
    ...
    static SerializeResult<SerializeError> serialize(value_type& value, BitVector<T>& bits, const U& parent = {})
    {
        if(value.size() > 0)
        {
            for(size_t i = 0; i < value.size(); ++i)
            {
                ...
                if(!element) return {SerializeElementError{i, std::move(element.error())}};
                if(i < value.size() - 1)
                {
                    value_type_t<SeparatorType> v;
                    auto                        sep = SeparatorType::serialize(v, bits, parent);
                    if(!sep) return {std::move(sep.error())};
                }
            }
        }
        return {};
