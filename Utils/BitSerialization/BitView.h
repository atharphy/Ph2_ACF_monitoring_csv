#ifndef BITSERIALIZATION_BIT_VIEW_H
#define BITSERIALIZATION_BIT_VIEW_H
...
class BitView
{
  ...
  public:
    ...
    {
        if(start > size()) throw std::out_of_range("BitView::slice: out of range.");
        return {_data, _start + start, _end};
    }

    BitView slice(size_t start, size_t end) const
    {
        if(start > end || end > size()) { throw std::out_of_range("BitView::slice: out of range."); }
