...
template <size_t N>
using uint_t = std::conditional_t<N <= 8, uint8_t, std::conditional_t<N <= 16, uint16_t, std::conditional_t<N <= 32, uint32_t, uint64_t>>>;
...
constexpr auto to_string_literal()
{
    ...
    *--ptr             = '\0';
    for(auto n = N < 0 ? -N : N; n; n /= base) *--ptr = digits[n % base];
    if(N < 0) *--ptr = '-';
...
{
    using type = std::conditional_t<Predicate<T>::value, tuple_prepend_t<T, filtered_tuple_t<Predicate, Ts...>>, filtered_tuple_t<Predicate, Ts...>>;
};
...
{
    using type = std::conditional_t<Predicate<T>::value, index_seq_prepend_t<I, typename indices_where<Predicate, I + 1, Ts...>::type>, typename indices_where<Predicate, I + 1, Ts...>::type>;
};
...
struct LookUpTableTransform
{
    ...
    static auto init_backward_map() { return init_backward_map(std::make_index_sequence<size>()); }

    output_type encode(const input_type& value) const { return forward_map[value]; }
...
template <class T, T... Values>
const typename LookUpTableTransform<T, Values...>::backward_map_map_type LookUpTableTransform<T, Values...>::backward_map = LookUpTableTransform<T, Values...>::init_backward_map();
