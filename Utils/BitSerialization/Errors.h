...
struct VoidError
{
    friend std::ostream& operator<<(std::ostream& os, const VoidError&) { return (os << "Generic error (no message)."); }
};
...
struct SizeError
{
    ...
    friend std::ostream& operator<<(std::ostream& os, const SizeError& error) { return (os << &TypeName.value[0] << " error: not enough bits (" << error.size << ")."); }
};
...
struct ValueError
{
    ...
    friend std::ostream& operator<<(std::ostream& os, const ValueError& error) { return (os << &TypeName.value[0] << " error: invalid value (" << std::ref(error.value) << ")."); }
};
...
struct ElementError
{
    ...
    ElementError(size_t index, SubError&& error) : index(index), error(std::move(error)) {}
...
struct isVariantMember<T, _ErrorVariant<ALL_T...>> : public std::disjunction<std::is_same<T, ALL_T>...>
{
};
...
struct _ErrorVariant
{
    ...
    _ErrorVariant(_ErrorVariant&& other) noexcept : storage(std::move(other.storage)) {}

    template <class Error, typename std::enable_if_t<isVariantMember<Error, _ErrorVariant>::value, int> = 0>
    _ErrorVariant(Error&& error) : storage(std::forward<Error>(error))
    {
    }
...
{
    using type = std::conditional_t<(std::is_same_v<T, Us> || ...), typename variant_append<_ErrorVariant<Us...>, Ts...>::type, typename variant_append<_ErrorVariant<Us..., T>, Ts...>::type>;
};
