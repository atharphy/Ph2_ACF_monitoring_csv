...
class CharArray //: public streammable
{
  public:
    ...
    void setString(const std::string& theString)
    {
        ...
        {
            std::string errorMessage = std::string(__PRETTY_FUNCTION__) + " " + std::to_string(__LINE__) + " size of registerName is greater that MAX_LENGHT_PARAMETER_STRING (" + std::to_string(MAX_LENGHT_PARAMETER_STRING) + ")"; 
