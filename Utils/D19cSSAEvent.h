...
{
  public:
    D19cSSAEvent(const Ph2_HwDescription::BeBoard* pBoard, uint32_t pNSSA, uint32_t pNHybrid, const std::vector<uint32_t>& list);
    ~D19cSSAEvent() {}
    ...
    std::string       DataHexString(uint8_t pHybridId, uint8_t pSSAId) const override;
    bool              Error(uint8_t pHybridId, uint8_t pSSAId, uint32_t i) const override;
    uint32_t          Error(uint8_t pHybridId, uint8_t pSSAId) const override;
    uint32_t          PipelineAddress(uint8_t pHybridId, uint8_t pSSAId) const override;
    std::string       DataBitString(uint8_t pHybridId, uint8_t pSSAId) const override;
    std::vector<bool> DataBitVector(uint8_t pHybridId, uint8_t pSSAId) const override;
    std::vector<bool> DataBitVector(uint8_t pHybridId, uint8_t pSSAId, const std::vector<uint8_t>& channelList) const override;
    std::string       GlibFlagString(uint8_t pHybridId, uint8_t pSSAId) const override;
    std::string       StubBitString(uint8_t pHybridId, uint8_t pSSAId) const override;
    bool              StubBit(uint8_t pHybridId, uint8_t pSSAId) const override;
    std::vector<Stub> StubVector(uint8_t pHybridId, uint8_t pSSAId) const override;
    uint32_t          GetNHits(uint8_t pHybridId, uint8_t pSSAId) const override;
    uint32_t          GetL1Number() const;
    uint32_t          GetTrigID() const;

    uint32_t              GetSSAL1Counter(uint8_t pHybridId, uint8_t pSSAId) const;
    std::vector<uint32_t> GetHits(uint8_t pHybridId, uint8_t pSSAId) const override;
    std::vector<Cluster>  getClusters(uint8_t pHybridId, uint8_t pSSAId) const override;
    void                  print(std::ostream& out) const override;
    bool                  DataBit(uint8_t pHybridId, uint8_t pSSAId, uint32_t i) const override { return privateDataBit(pHybridId, pSSAId, i); };
    inline bool           privateDataBit(uint8_t pHybridId, uint8_t pSSAId, uint8_t i) const
    {
        try
        {
            return (fEventDataVector.at(encodeVectorIndex(pHybridId, pSSAId, fNSSA)).at(calculateChannelWordPosition(119 - i)) >> (calculateChannelBitPosition(119 - i))) & 0x1;
        }
        catch(const std::out_of_range& outOfRange)
        {
            LOG(ERROR) << "Word " << +i << " for Hybrid " << +pHybridId << " SSA " << +pSSAId << " is not found:";
