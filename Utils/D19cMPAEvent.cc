    ...
    for(auto cOpticalGroup: *pBoard)
    {
        for(auto cHybrid: *cOpticalGroup)
        {
            for(auto cChip: *cHybrid)
            {
                ...
                if(cPLeadingMPA != 0xA && cPLeadingMPA != 0x0) LOG(ERROR) << "Incorrect L1A header for MPA " << unsigned(pMPAId)<<","<<unsigned(cHybrid->getId())<<","<<unsigned(cOpticalGroup->getId());
                if(cSLeadingMPA != 0x5 && cSLeadingMPA != 0x0) LOG(ERROR) << "Incorrect stub header for MPA " << unsigned(pMPAId)<<","<<unsigned(cHybrid->getId())<<","<<unsigned(cOpticalGroup->getId());
