...
{
D19cSSAEvent::D19cSSAEvent(const BeBoard* pBoard, uint32_t pNSSA, uint32_t pNHybrid, const std::vector<uint32_t>& list) : fEventDataVector(pNSSA * pNHybrid)
{
    fNSSA = pNSSA;
...
void D19cSSAEvent::SetEvent(const BeBoard* pBoard, uint32_t pNSSA, const std::vector<uint32_t>& list)
{
    ...
    for(uint32_t chip = 0; chip < pNSSA; chip++)
    {
        ...
        // save it
        fEventDataVector[encodeVectorIndex(cHybridId, cSSAId, pNSSA)] = lvec;
        // LOG(INFO) << BOLDBLUE <<"SETTING "<<encodeVectorIndex(cHybridId, cSSAId, pNSSA)<< " " << cHybridId<< " " <<cSSAId<< "
        // " <<pNSSA<< RESET;
    }
}
std::string D19cSSAEvent::HexString() const { return ""; }
std::string D19cSSAEvent::DataHexString(uint8_t pHybridId, uint8_t pSSAId) const { return ""; }
bool        D19cSSAEvent::Error(uint8_t pHybridId, uint8_t pSSAId, uint32_t i) const // FIXME NOT WORKING?!
{
    return Bit(pHybridId, pSSAId, D19C_OFFSET_ERROR_CBC3);
}
uint32_t D19cSSAEvent::Error(uint8_t pHybridId, uint8_t pSSAId) const
{
    try
    {
        const std::vector<uint32_t>& hitVector = fEventDataVector.at(encodeVectorIndex(pHybridId, pSSAId, fNCbc));
    ...
    {
        LOG(ERROR) << "Word 4 for Hybrid " << +pHybridId << " SSA " << +pSSAId << " is not found:";
        LOG(ERROR) << "Out of Range error: " << outOfRange.what();
        return 0;
    }
}
uint32_t D19cSSAEvent::PipelineAddress(uint8_t pHybridId, uint8_t pCbcId) const
{
    LOG(ERROR) << "This is not a CBC, this is an SSA!" << RESET;
    assert(false);
    return 0;
}
std::string D19cSSAEvent::DataBitString(uint8_t pHybridId, uint8_t pSSAId) const
{
    std::ostringstream os;
    for(uint32_t i = 0; i < NSSACHANNELS; ++i) { os << privateDataBit(pHybridId, pSSAId, i); }
    return os.str();
}
std::vector<bool> D19cSSAEvent::DataBitVector(uint8_t pHybridId, uint8_t pSSAId) const
{
    std::vector<bool> blist;
    for(uint32_t i = 0; i < NSSACHANNELS; ++i) { blist.push_back(privateDataBit(pHybridId, pSSAId, i)); }

    return blist;
}
std::vector<bool> D19cSSAEvent::DataBitVector(uint8_t pHybridId, uint8_t pSSAId, const std::vector<uint8_t>& channelList) const
{
    std::vector<bool> blist;

    for(auto i: channelList) { blist.push_back(privateDataBit(pHybridId, pSSAId, i)); }
    return blist;
}
std::string D19cSSAEvent::GlibFlagString(uint8_t pHybridId, uint8_t pSSAId) const { return ""; }
std::string D19cSSAEvent::StubBitString(uint8_t pHybridId, uint8_t pSSAId) const // FIXME
{
    LOG(ERROR) << "This is not a CBC, this is an SSA!" << RESET;
    assert(false);
    return "";
}
bool D19cSSAEvent::StubBit(uint8_t pHybridId, uint8_t pSSAId) const
{
    LOG(ERROR) << "This is not a CBC, this is an SSA!" << RESET;
    assert(false);
    return false;
}
std::vector<Stub> D19cSSAEvent::StubVector(uint8_t pHybridId, uint8_t pSSAId) const
{
    LOG(ERROR) << "This is not a CBC, this is an SSA!" << RESET;
    assert(false);
    return std::vector<Stub>();
}
uint32_t D19cSSAEvent::GetNHits(uint8_t pHybridId, uint8_t pSSAId) const
{
    try
    {
        uint32_t                     cHits     = 0;
    ...
    {
        LOG(ERROR) << "Hits for Hybrid " << +pHybridId << " SSA " << +pSSAId << " is not found:";
        LOG(ERROR) << "Out of Range error: " << outOfRange.what();
        return 0;
    }
}
...
uint32_t D19cSSAEvent::GetSSAL1Counter(uint8_t pHybridId, uint8_t pSSAId) const
{
    const std::vector<uint32_t>& hitVector = fEventDataVector.at(encodeVectorIndex(pHybridId, pSSAId, fNCbc));
    return (hitVector.at(6) & 0x000F0000) >> 16;
}
std::vector<uint32_t> D19cSSAEvent::GetHits(uint8_t pHybridId, uint8_t pSSAId) const
{
    std::vector<uint32_t> CA(8);
    std::vector<uint32_t> stubs;

    // LOG(INFO) << BOLDBLUE <<"ACCESSING "<<encodeVectorIndex(pHybridId, pSSAId, fNSSA)<< " " << uint32_t(pHybridId)<< " "
    // <<uint32_t(pSSAId)<< " " <<uint32_t(fNSSA)<< RESET;

    std::vector<uint32_t> lvec = fEventDataVector[encodeVectorIndex(pHybridId, pSSAId, fNSSA)];
    ...
    //{
    //	cHits.push_back(privateDataBit(pHybridId, pSSAId, i));
    //}
    // return cHits;
}
void D19cSSAEvent::print(std::ostream& os) const // TODO add info here as needed
{
    ...
    for(__attribute__((unused)) auto const& hitVector: fEventDataVector)
    {
        ...
        os << " HybridId: " << +cHybridId << " SSAId: " << +cSSAId << RESET << std::endl;
    }
    os << std::endl;
}
std::vector<Cluster> D19cSSAEvent::getClusters(uint8_t pHybridId, uint8_t pSSAId) const
{
    LOG(ERROR) << "This is not a CBC, this is an SSA!" << RESET;
    assert(false);
