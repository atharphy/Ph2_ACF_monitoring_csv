...
class EmptyContainer //: public streammable
{
  public:
    ...
    template <typename T>
    uint32_t makeChannelAverage(const ChipContainer* theChipContainer, const std::shared_ptr<ChannelGroupBase> chipOriginalMask, const std::shared_ptr<ChannelGroupBase> cTestChannelGroup, const uint32_t numberOfEvents)
    {
        return 0;
    }
    template <typename T>
