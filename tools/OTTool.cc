...
void OTTool::ContinousReadoutTh(uint8_t cBrdId)
{
    ...
    if(cData.size() != 0) std::move(cData.begin(), cData.end(), std::back_inserter(fReadoutData[cBrdId]));
    LOG (INFO) << BOLDYELLOW << "BeBoard#" << +cBrdId << " readData found " << cLclEvntCntr << " events...\tAccumulated " << cAccumulatedWaits * 1e-3 << " ms of waits.. read-out " << fReadoutData[cBrdId].size() << " 32-bit words in " << cDuration << " ms." << RESET;
