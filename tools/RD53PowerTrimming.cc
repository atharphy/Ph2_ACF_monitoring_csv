...
{
    // Retreive target current values from settings, or use defaults if not found
        ...
        for(const auto cOpticalGroup: *cBoard)
            for(const auto cHybrid: *cOpticalGroup)
                for(const auto cChip: *cHybrid)
                {
                    ...
                    std::vector<std::string> comp_registers = {
                        "DAC_COMP_LIN"
                    };
                    std::vector<std::string> preamp_registers = {
                        "DAC_PREAMP_M_LIN" ,
                        "DAC_PREAMP_R_LIN" ,
                        "DAC_PREAMP_L_LIN" ,
                        "DAC_PREAMP_T_LIN" ,
                    ...
                    std::vector<std::string> ldac_registers = {
                        "DAC_LDAC_LIN"
                    };
                    ...
                    for (const auto& regName : comp_registers)
                        fReadoutChipInterface->WriteChipReg(theChip, regName, 0, true);
                    for (const auto& regName : ldac_registers)
                        fReadoutChipInterface->WriteChipReg(theChip, regName, 0, true);

                    // PREAMP scan
                    thePreamplifierCurrentContainer.getObject(
                        cBoard->getId())->getObject(cOpticalGroup->getId())->getObject(cHybrid->getId())->getObject(cChip->getId()
                    )->getSummary<dataType>() = linearScanBottomUp(theChip, preamp_registers, 0, MAX_PREAMP, "ANA_IN_CURR", PREAMP_CURRENT_mA);

                    // COMP scan
                    theComparatorCurrentContainer.getObject(
                        cBoard->getId())->getObject(cOpticalGroup->getId())->getObject(cHybrid->getId())->getObject(cChip->getId()
                    )->getSummary<dataType>() = linearScanBottomUp(theChip, comp_registers, 0, MAX_COMP, "ANA_IN_CURR", COMP_CURRENT_mA);
                    ...
                    theLDACCurrentContainer.getObject(
                        cBoard->getId())->getObject(cOpticalGroup->getId())->getObject(cHybrid->getId())->getObject(cChip->getId()
                    )->getSummary<dataType>() = linearScanBottomUp(theChip, ldac_registers, 0, MAX_LDAC, "ANA_IN_CURR", LDAC_CURRENT_mA);
...
dataType PowerTrimming::linearScanBottomUp(RD53* chip, const std::vector<std::string>& regNames, uint16_t startValue, uint16_t maxValue, const std::string targetName, float& targetDiff)
{
    auto WriteChipRegisters = [this, chip, regNames](uint16_t value)
    {
        for (const auto& regName : regNames)
            fReadoutChipInterface->WriteChipReg(chip, regName, value, false);
    };
    
    std::vector<std::pair<std::string, uint16_t>> regDefaults;
    for (const auto& regName : regNames)
        regDefaults.push_back({regName, fReadoutChipInterface->ReadChipReg(chip, regName)});

    std::stringstream _regNames;
    for (const auto& regName : regNames)
    {
        _regNames << regName;
        if (&regName != &regNames.back())
            _regNames << " ";
    }

    LOG(INFO) << BOLDYELLOW 
        << "chip " << chip->geteFuseCode()          << BOLDGREEN 
        << ": starting a linear scan from "         << BOLDYELLOW << startValue << BOLDGREEN 
        << " to reach target current increase of "  << BOLDYELLOW << targetDiff << BOLDGREEN
        << " mA for registers "                     << BOLDYELLOW << _regNames.str() << RESET;
    ...
    float monitor       = 1.0e-3 * RD53Constants::CURRENT_FACTOR * fReadoutChipInterface->ReadChipMonitor(chip, targetName, true);
    ...
    while (set_diff > 0.0)
    {
        ...
        {
            LOG(WARNING) << BOLDYELLOW 
                <<"Reached the maximum allowed value for the registers " << BOLDMAGENTA 
                << _regNames.str()                                       << BOLDYELLOW 
                << " at "                                                << BOLDMAGENTA 
                << set_value                                             << BOLDYELLOW
                << ". Stopping the scan."                                << RESET;
            break;
        }
        WriteChipRegisters(set_value);
        monitor   = 1.e-3 * RD53Constants::CURRENT_FACTOR * fReadoutChipInterface->ReadChipMonitor(chip, targetName, true);
        ...
        // Read all relevant ADC monitors (Currents and Tensions) at each step of the scan and save them in the result vector and in a txt file for monitoring
        PTData.timestamp = seconds.count();
        PTData.bit = set_value;
        PTData.ANA_IN_CURR = 1.e-3 * RD53Constants::CURRENT_FACTOR * fReadoutChipInterface->ReadChipMonitor(chip, "ANA_IN_CURR", true);
        PTData.DIG_IN_CURR = 1.e-3 * RD53Constants::CURRENT_FACTOR * fReadoutChipInterface->ReadChipMonitor(chip, "DIG_IN_CURR", true);
        PTData.VINA = 4.0 * fReadoutChipInterface->ReadChipMonitor(chip, "VINA", true);
        PTData.VDDA = 2.0 * fReadoutChipInterface->ReadChipMonitor(chip, "VDDA", true);
        PTData.VIND = 4.0 * fReadoutChipInterface->ReadChipMonitor(chip, "VIND", true);
        PTData.VDDD = 2.0 * fReadoutChipInterface->ReadChipMonitor(chip, "VDDD", true);
        PTData.Iref = fReadoutChipInterface->ReadChipMonitor(chip, "Iref", true);
        PTData.ANA_SHUNT_CURR = 1.e-3 * RD53Constants::SHUNT_CURRENT_FACTOR * fReadoutChipInterface->ReadChipMonitor(chip, "ANA_SHUNT_CURR", true);
        PTData.DIG_SHUNT_CURR = 1.e-3 * RD53Constants::SHUNT_CURRENT_FACTOR * fReadoutChipInterface->ReadChipMonitor(chip, "DIG_SHUNT_CURR", true);
        ...
        if (outFile.is_open())
        {
            outFile << std::put_time(&tm, "%H:%M:%S") << "\t"
                    ...
                    << PTData.VDDD << "\t"
                    << PTData.Iref << "\t"
                    << PTData.ANA_SHUNT_CURR << "\t"
                    << PTData.DIG_SHUNT_CURR << "\n";
        }
    ...
    float digcurr = 1.e-3 * RD53Constants::CURRENT_FACTOR * fReadoutChipInterface->ReadChipMonitor(chip, "DIG_IN_CURR", true);
    LOG(INFO) << BOLDYELLOW 
        << "chip " << chip->geteFuseCode()                   << BOLDGREEN
        << ": scan ended. Found value "                      << BOLDYELLOW
        << result.back().first                               << BOLDGREEN
        << " with current " << targetName << " = "           << BOLDYELLOW
        << result.back().second                              << BOLDGREEN
        << " mA (difference from target: " << set_diff << " mA) // dig. current: " << digcurr << " mA" << RESET;

    // PREAMP requires specific calibration factors depending on the physical area of the matrix region.
    for (const auto& regName : regNames)
    {
        float calib_factor = 1.0;
        if      (regName == "DAC_PREAMP_M_LIN")  calib_factor = 1.0;
        else if (regName == "DAC_PREAMP_R_LIN")  calib_factor = RD53BConstants::PREAMP_R;
        else if (regName == "DAC_PREAMP_L_LIN")  calib_factor = RD53BConstants::PREAMP_L;
        else if (regName == "DAC_PREAMP_T_LIN")  calib_factor = RD53BConstants::PREAMP_T;
        else if (regName == "DAC_PREAMP_TR_LIN") calib_factor = RD53BConstants::PREAMP_TR;
        else if (regName == "DAC_PREAMP_TL_LIN") calib_factor = RD53BConstants::PREAMP_TL;
        
        uint16_t bit_value = result.back().first;
        int calib_value = std::round(bit_value * calib_factor);

        if (calib_value > MAX_PREAMP)
        {
            calib_value = MAX_PREAMP;
        }
        
        LOG(INFO) << BOLDGREEN << "Register: " << BOLDYELLOW << std::setw(18) << std::left << regName 
                  << BOLDGREEN << " | Value: " << BOLDYELLOW << calib_value << RESET;
        fReadoutChipInterface->WriteChipReg(chip, regName, calib_value, true);
    }
    ...
    if (regNames.front() == "DAC_COMP_LIN") 
    {
        ...
        uint16_t fc_value = fReadoutChipInterface->ReadChipReg(chip, "DAC_FC_LIN");
        uint16_t new_fc_value = std::round(fc_value * static_cast<float>(set_value) / comp_value);
        if (new_fc_value > 1023) new_fc_value = 1023;

        uint16_t comp_ta_value = fReadoutChipInterface->ReadChipReg(chip, "DAC_COMP_TA_LIN");
        uint16_t new_comp_ta_value = std::round(comp_ta_value * static_cast<float>(set_value) / comp_value);
        if (new_comp_ta_value > 1023) new_comp_ta_value = 1023;
