...
void MemoryCheck2S::Reset()
{
    ...
    for(auto cBoard: *fDetectorContainer)
    {
        ...
        auto& cRegMapThisBoard = fRegMapContainer.topoGigio(cBoard->getIndex());
        for(auto cOpticalGroup: *cBoard)
        {
            auto& cRegMapThisOpticalGroup = cRegMapThisBoard->topoGigio(cOpticalGroup->getIndex());
            for(auto cHybrid: *cOpticalGroup)
            {
                auto& cRegMapThisHybrid = cRegMapThisOpticalGroup->topoGigio(cHybrid->getIndex());
                ...
                {
                    auto&                                         cRegMapThisChip = cRegMapThisHybrid->topoGigio(cChip->getIndex())->getSummary<ChipRegMap>();
...
void MemoryCheck2S::Reconfigure()
{
    for(auto cBoard: *fDetectorContainer)
    {
        ...
        // // reconfigure masks
        auto& cMasksThisBrd = fChipMasks.topoGigio(cBoard->getIndex());
        LOG(INFO) << BOLDMAGENTA << "\t... [MemoryCheck2S] Resetting Chip masks back to their original values" << RESET;
        for(auto cOpticalGroup: *cBoard)
        {
            auto& cMasksThisOG = cMasksThisBrd->topoGigio(cOpticalGroup->getIndex());
            for(auto cHybrid: *cOpticalGroup)
            {
                auto& cMasksThisHybrid = cMasksThisOG->topoGigio(cHybrid->getIndex());
                for(auto cChip: *cHybrid)
                {
                    auto& cMasksThisChip = cMasksThisHybrid->topoGigio(cChip->getIndex());
    ...
    for(auto cBoard: *fDetectorContainer)
    {
        ...
        for(auto cOpticalGroup: *cBoard)
        {
            for(auto cHybrid: *cOpticalGroup)
            {
                for(auto cChip: *cHybrid)
                {
                    ChipRegMap&       theChipMap     = fRegMapContainer.topoGigio(cBoard->getIndex())->topoGigio(cOpticalGroup->getIndex())->topoGigio(cHybrid->getIndex())->topoGigio(cChip->getIndex())->getSummary<ChipRegMap>();
...
// to be replaced with a plotting tool
    ...
    for(auto cBoard: *fDetectorContainer)
    {
        auto& cPkgDelayThisBrd = fPackageDelays->topoGigio(cBoard->getIndex());
...
void MemoryCheck2S::EvaluatePedeNoise(int pNevents, int pScanRange)
{
    ...
    for(auto cBoard: *fDetectorContainer)
    {
        auto& cBeBoardSpars = cSparsBoards.topoGigio(cBoard->getIndex());
    ...
    for(auto cBoard: *fDetectorContainer)
    {
        auto& cThNoiseThisBrd = fThresholdAndNoiseContainer->topoGigio(cBoard->getIndex());
        for(auto cOpticalGroup: *cBoard)
        {
            auto& cThNoiseThisOG = cThNoiseThisBrd->topoGigio(cOpticalGroup->getIndex());
            for(auto cHybrid: *cOpticalGroup)
            {
                auto& cThNoiseThisHybrid = cThNoiseThisOG->topoGigio(cHybrid->getIndex());
                for(auto cChip: *cHybrid)
                {
                    if(cChip->getFrontEndType() != FrontEndType::CBC3) continue;

                    auto&              cThNoiseThisChip = cThNoiseThisHybrid->topoGigio(cChip->getIndex());
                    ...
                    for(size_t cChnl = 0; cChnl < cChip->size(); cChnl++)
                    {
                        ...
                        {
                            auto& cDataThisBrd    = cScanData[cIndx]->topoGigio(cBoard->getIndex());
                            auto& cDataThisOG     = cDataThisBrd->topoGigio(cOpticalGroup->getIndex());
                            auto& cDataThisHybrid = cDataThisOG->topoGigio(cHybrid->getIndex());
                            auto& cDataThisChip   = cDataThisHybrid->topoGigio(cChip->getIndex());
    ...
    for(auto cBoard: *fDetectorContainer)
    {
        auto& cBeBoardSpars = cSparsBoards.topoGigio(cBoard->getIndex())->getSummary<uint8_t>();
...
void MemoryCheck2S::SetThreshold(float pSigma)
{
    for(auto cBoard: *fDetectorContainer)
    {
        auto& cThThisBoard    = fThresholds.topoGigio(cBoard->getIndex());
        auto& cThNoiseThisBrd = fThresholdAndNoiseContainer->topoGigio(cBoard->getIndex());
        for(auto cOpticalGroup: *cBoard)
        {
            auto& cThThisOG      = cThThisBoard->topoGigio(cOpticalGroup->getIndex());
            auto& cThNoiseThisOG = cThNoiseThisBrd->topoGigio(cOpticalGroup->getIndex());
            for(auto cHybrid: *cOpticalGroup)
            {
                auto& cThThisHybrid      = cThThisOG->topoGigio(cHybrid->getIndex());
                auto& cThNoiseThisHybrid = cThNoiseThisOG->topoGigio(cHybrid->getIndex());
...
void MemoryCheck2S::DataCheck(std::vector<uint8_t> pActiveCbcs, int pMeanTriggerSeparation, bool pAllOnes)
{
    ...
    for(auto cBoard: *fDetectorContainer)
    {
        auto& cExpectdStubsThisBoard = fExpectedStubs.topoGigio(cBoard->getIndex());
        auto& cExpectedOccThisBoard  = fExpectedOccupancy.topoGigio(cBoard->getIndex());
        for(auto cOpticalGroup: *cBoard)
        {
            auto& cExpectedStubsThisOG = cExpectdStubsThisBoard->topoGigio(cOpticalGroup->getIndex());
            auto& cExpectedOccThisOG   = cExpectedOccThisBoard->topoGigio(cOpticalGroup->getIndex());
            for(auto cHybrid: *cOpticalGroup)
            {
                auto& cExpectedStubsThisHybrid = cExpectedStubsThisOG->topoGigio(cHybrid->getIndex());
                auto& cExpectedOccThisHybrid   = cExpectedOccThisOG->topoGigio(cHybrid->getIndex());
                // only 2S for now
                // configure injection
                for(auto cChip: *cHybrid)
                {
                    auto& cExpectedStubsThisChip = cExpectedStubsThisHybrid->topoGigio(cChip->getIndex());
                    auto& cExpectedStubs         = cExpectedStubsThisChip->getSummary<std::vector<Stub>>();
                    cExpectedStubs.clear();

                    auto& cExpectedOccThisChip = cExpectedOccThisHybrid->topoGigio(cChip->getIndex());
                    if(cChip->getFrontEndType() != FrontEndType::CBC3) continue;
...
void MemoryCheck2S::SaveOptimalTaps()
{
    ...
    for(auto cBoard: *fDetectorContainer)
    {
        // auto& cVrefCorrThisBoard = fVrefCorrections.topoGigio(cBoard->getIndex());
        for(auto cOpticalGroup: *cBoard)
        {
            for(auto cHybrid: *cOpticalGroup)
            {
...
void MemoryCheck2S::ConfigureVref()
{
    ...
    for(auto cBoard: *fDetectorContainer)
    {
        ...
        for(auto cOpticalGroup: *cBoard)
        {
            ...
            // auto& cVrefCorrThisOG = cVrefCorrThisBoard->topoGigio(cOpticalGroup->getIndex());
...
void MemoryCheck2S::MonitorAnalogue()
{
    ...
    for(size_t cMuxIndx = 0; cMuxIndx < cAmuxSels.size(); cMuxIndx++)
    {
        ...
        for(const auto cBoard: *fDetectorContainer)
        {
            auto& cBiasThisBrd     = cAnBiases.topoGigio(cBoard->getIndex());
            auto& cMeasThisBrd     = cAdcMeasurements.topoGigio(cBoard->getIndex());
            auto& cFloatingThisBrd = cFloatingMonitor.topoGigio(cBoard->getIndex());
            // now for the analogue monitoring
            for(auto cOpticalGroup: *cBoard)
            {
                uint8_t cVrefCorr = fVrefCorrections[cCounter];
                cCounter++;
                auto& cBiasThisOG     = cBiasThisBrd->topoGigio(cOpticalGroup->getIndex());
                auto& cMeasThisOG     = cMeasThisBrd->topoGigio(cOpticalGroup->getIndex());
                auto& cFloatingThisOG = cFloatingThisBrd->topoGigio(cOpticalGroup->getIndex());
                auto& clpGBT          = cOpticalGroup->flpGBT;
                if(clpGBT == nullptr) continue;
                auto clpGBTInterface = static_cast<D19clpGBTInterface*>(flpGBTInterface);
                ...
                for(size_t cChipIndx = 0; cChipIndx < 8; cChipIndx++)
                {
                    ...
                    for(size_t cMeasIndx = 0; cMeasIndx < cLengthLoop * 2; cMeasIndx++)
                    {
                        ...
                        for(auto cHybrid: *cOpticalGroup)
                        {
                            auto& cFltThisHybrid = cFloatingThisOG->topoGigio(cHybrid->getIndex());
                    ...
                    for(auto cHybrid: *cOpticalGroup)
                    {
                        ...
                        fReadoutChipInterface->WriteChipReg(cHybrid->topoGigio(cChipIndx), "AmuxOutput", cAmuxSels[cMuxIndx]); // set to floating
                    }
                    // allow to stabilize
                    for(size_t cMeasIndx = 0; cMeasIndx < cLengthLoop / 10; cMeasIndx++)
                    {
                        ...
                        for(auto cHybrid: *cOpticalGroup)
                        {
                            auto& cMeasThisHybrid = cMeasThisOG->topoGigio(cHybrid->getIndex());
                            auto& cMeasThisChip   = cMeasThisHybrid->topoGigio(cChipIndx);
                    ...
                    for(auto cHybrid: *cOpticalGroup)
                    {
                        ...
                        //
                        auto&   cBiasThisHybrid      = cBiasThisOG->topoGigio(cHybrid->getIndex());
                        auto&   cBiasThisChip        = cBiasThisHybrid->topoGigio(cChipIndx);
                        ...
                        fADCmeasurement.fChipId      = (cHybrid->topoGigio(cChipIndx))->getId();
                        fADCmeasurement.fMean        = cStats.fMean;
                        fADCmeasurement.fStdDev      = cStats.fStdDev;
                        fADCmeasurement.fScaling     = 1;
                        fADCmeasurement.fVrefCorr    = cVrefCorr;
                    ...
                    {
                        if(cChipIndx >= cHybrid->size()) continue;
                        fReadoutChipInterface->WriteChipReg(cHybrid->topoGigio(cChipIndx), "AmuxOutput", 0); // set to floating
                    }
...
void MemoryCheck2S::Check()
{
    ...
    for(auto cBoard: *fDetectorContainer)
    {
        ...
        auto&                      cThThisBoard       = fThresholds.topoGigio(cBoard->getIndex());
        auto&                      cThNoiseThisBrd    = fThresholdAndNoiseContainer->topoGigio(cBoard->getIndex());
        auto&                      cMemEventsThisBrd  = cMemEvents.topoGigio(cBoard->getIndex());
        auto&                      cBadEventsThisBrd  = cCorruptedEvents.topoGigio(cBoard->getIndex());
        auto&                      cExpectedOcThisBrd = fExpectedOccupancy.topoGigio(cBoard->getIndex());
        ...
        for(auto& cEvent: cEvents)
        {
            ...
            for(auto cOpticalGroup: *cBoard)
            {
                auto& cThThisOG         = cThThisBoard->topoGigio(cOpticalGroup->getIndex());
                auto& cThNoiseThisOG    = cThNoiseThisBrd->topoGigio(cOpticalGroup->getIndex());
                auto& cMemEventsThisOG  = cMemEventsThisBrd->topoGigio(cOpticalGroup->getIndex());
                auto& cBadEventsThisOG  = cBadEventsThisBrd->topoGigio(cOpticalGroup->getIndex());
                auto& cExpectedOcThisOG = cExpectedOcThisBrd->topoGigio(cOpticalGroup->getIndex());
                for(auto cHybrid: *cOpticalGroup)
                {
                    auto& cThThisHybrid         = cThThisOG->topoGigio(cHybrid->getIndex());
                    auto& cThNoiseThisHybrid    = cThNoiseThisOG->topoGigio(cHybrid->getIndex());
                    auto& cMemEventsThisHybrid  = cMemEventsThisOG->topoGigio(cHybrid->getIndex());
                    auto& cBadEventsThisHybrid  = cBadEventsThisOG->topoGigio(cHybrid->getIndex());
                    auto& cExpectedOcThisHybrid = cExpectedOcThisOG->topoGigio(cHybrid->getIndex());
                    // only 2S for now
                    for(auto cChip: *cHybrid)
                    {
                        if(cChip->getFrontEndType() != FrontEndType::CBC3) continue;
                        ...
                        auto& cThNoiseThisChip   = cThNoiseThisHybrid->topoGigio(cChip->getIndex());
                        auto& cMemEventsThisChip = cMemEventsThisHybrid->topoGigio(cChip->getIndex());
                        auto& cMemEventsSummary  = cMemEventsThisChip->getSummary<MemEvents>();

                        auto& cBadEventsThisChip = cBadEventsThisHybrid->topoGigio(cChip->getIndex());
                        auto& cBadEventsSummary  = cBadEventsThisChip->getSummary<MemEvents>();

                        if(cEvntCnt == 0) cMemEventsSummary.clear();

                        auto& cExpectedOcThisChip = cExpectedOcThisHybrid->topoGigio(cChip->getIndex());
                        //
                        ...
                        if(fMemEvent.fType == 3)
                        {
                            ...
                            auto& cExpectdStubsThisBoard  = fExpectedStubs.topoGigio(cBoard->getIndex());
                            auto& cExpectdStubsThisOG     = cExpectdStubsThisBoard->topoGigio(cOpticalGroup->getIndex());
                            auto& cExpectdStubsThisHybrid = cExpectdStubsThisOG->topoGigio(cHybrid->getIndex());
                            auto& cExpectdStubsThisChip   = cExpectdStubsThisHybrid->topoGigio(cChip->getIndex());
