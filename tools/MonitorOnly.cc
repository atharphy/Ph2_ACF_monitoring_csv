...
void MonitorOnly::signalHandler(int signal)
{
    ...
    if(fInstance) { fInstance->fKeepMonitoring.store(false); }
}
...
MonitorOnly::~MonitorOnly()
{
    ...
    if(fCommandThread.joinable()) { fCommandThread.join(); }
...
void MonitorOnly::createNamedPipes()
{
    ...
    else { LOG(INFO) << BOLDGREEN << __PRETTY_FUNCTION__ << " Created data pipe: " << fDataPipeName << RESET; }
    ...
    else { LOG(INFO) << BOLDGREEN << __PRETTY_FUNCTION__ << " Created command pipe: " << fCommandPipeName << RESET; }
}
...
void MonitorOnly::cleanupNamedPipes()
{
    ...
    // Remove named pipes
    if(unlink(fDataPipeName.c_str()) == 0) { LOG(DEBUG) << BOLDGREEN << __PRETTY_FUNCTION__ << " Removed data pipe: " << fDataPipeName << RESET; }
    if(unlink(fCommandPipeName.c_str()) == 0) { LOG(DEBUG) << BOLDGREEN << __PRETTY_FUNCTION__ << " Removed command pipe: " << fCommandPipeName << RESET; }
}
        ...
        if(!fPaused.load())
        {
            ...
            for(const auto& board: *fDetectorContainer)
            {
                for(const auto& opticalGroup: *board)
                {
                    ...
                    if(fuseIdCache.find(ogId) == fuseIdCache.end()) { fuseIdCache[ogId] = flpGBTInterface->ReadChipFuseID(theLpGBT); }
                    ...
                    try
                    {
                            ...
                            if(!opticalGroup->getNTCMap().empty())
                            {
                                ...
                                for(const auto& ntcEntry: ntcMap)
                                {
                                    if(ntcEntry.first == "Sensor")
                                    {
                                        try
                                        {
                                            flpGBTInterface->CdacSetCurrent(theLpGBT, ntcEntry.second, flpGBTInterface->_CdacCodeToCurrent(theLpGBT, ntcEntry.second, 0xaa));
                        ...
                        for(const auto& hybrid: *opticalGroup)
                        {
                            for(const auto& chip: *hybrid)
                            {
                                try
                                {
                                    ...
                                    if(chip->getFrontEndType() == FrontEndType::SSA2)
                                    {
                                        ...
                                        if(thePSInterface != nullptr)
                                        {
                                            ...
                                            if(counter < 3) { LOG(INFO) << BOLDMAGENTA << __PRETTY_FUNCTION__ << " SSA chip " << chip->getId() << " temp: " << chipTemp << RESET; }
                                        }
                                    }
                                    else if(chip->getFrontEndType() == FrontEndType::MPA2)
                                    {
                                        ...
                                        if(thePSInterface != nullptr)
                                        {
                                            chipTemp = thePSInterface->measureTemperature(chip);
                                            // Debug output for first few readings
                                            if(counter < 3) { LOG(INFO) << BOLDMAGENTA << __PRETTY_FUNCTION__ << " MPA chip " << chip->getId() << " temp: " << chipTemp << RESET; }
                                        }
                                    }
                                ...
                                {
                                    std::string chipType = (chip->getFrontEndType() == FrontEndType::SSA2) ? "SSA" : (chip->getFrontEndType() == FrontEndType::MPA2) ? "MPA" : "UNK";
                                    std::string hybridId = std::to_string(hybrid->getId());
                                    std::string chipId   = std::to_string(chip->getId());
                    ...
                    if(!pipe_available && !fDataPipe.is_open())
                    {
                        ...
                        else if(errno != ENXIO)
                        {
                            ...
                            {
                                LOG(DEBUG) << BOLDYELLOW << __PRETTY_FUNCTION__ << " Pipe open attempt failed (error count: " << open_error_count << ", errno: " << errno << ")" << RESET;
                            }
                        }
                    }
            ...
            if(counter % 3 == 0) { LOG(INFO) << BOLDMAGENTA << __PRETTY_FUNCTION__ << " Sent " << counter << " temperature data packets" << RESET; }
        }
                ...
                while(fKeepMonitoring.load())
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            if(fCommandPipe.eof() || fCommandPipe.fail()) { break; }
                        }
                    }
                    catch(const std::exception& e)
                    {
