...
void BeamTestCheck::CheckWithExternal(uint8_t pContinousReadout)
{
    ...
    if(fScanStubLatency) ScanStubLatency(pContinousReadout);
        ...
        for(auto cBoard: *fDetectorContainer)
        {
            LOG(INFO) << BOLDYELLOW << "BeBoard#" << +cBoard->getId() << " optimal stub latency found to be " << fOptimalStubLatency.at(cBoard->getGlobalIndex())->getSummary<uint32_t>() << RESET;
            for(auto cOpticalGroup: *cBoard) // for on opticalGroup - begin
            {
                for(auto cHybrid: *cOpticalGroup) // for on hybrid - begin
                {
                    ...
                    {
                        auto& cLat =
        fOptimalL1Latency.at(cBoard->getGlobalIndex())->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getSummary<uint16_t>(); LOG(INFO) << BOLDYELLOW
...
void BeamTestCheck::UpdateClusterContainers(BeBoard* pBoard, const std::vector<Event*> pEvents, size_t pIndx)
{
    ...
    if(fThStep % 10 == 0) LOG(INFO) << BOLDMAGENTA << "Calculating cluster occupancy for " << +pEvents.size() << " events read-back from BeBoard#" << +pBoard->getGlobalIndex() << RESET;
    auto   cClusterOccupancy    = fClusterOccupancy.at(pBoard->getGlobalIndex());
    auto   cClusterOccupancyS0  = fClusterOccupancyS0.at(pBoard->getGlobalIndex());
    auto   cClusterOccupancyS1  = fClusterOccupancyS1.at(pBoard->getGlobalIndex());
    ...
    for(size_t cTriggerId = 0; cTriggerId < cTriggerMult + 1; cTriggerId++)
    {
        ...
        do
        {
            ...
            {
                auto& cClusterOccupancyOG   = cClusterOccupancy->at(cOpticalGroup->getGlobalIndex());
                auto& cClusterOccupancyOGS0 = cClusterOccupancyS0->at(cOpticalGroup->getGlobalIndex());
                auto& cClusterOccupancyOGS1 = cClusterOccupancyS1->at(cOpticalGroup->getGlobalIndex());
                for(auto cHybrid: *cOpticalGroup)
                {
                    auto& cClusterOccupancyH   = cClusterOccupancyOG->at(cHybrid->getGlobalIndex());
                    auto& cClusterOccupancyHS0 = cClusterOccupancyOGS0->at(cHybrid->getGlobalIndex());
                    auto& cClusterOccupancyHS1 = cClusterOccupancyOGS1->at(cHybrid->getGlobalIndex());

                    for(auto cChip: *cHybrid)
                    {
                        if(cChip->getFrontEndType() != FrontEndType::CBC3) continue;

                        // auto cPedestal = fPedestalContainer.at(pBoard->getGlobalIndex())
                        //                     ->at(cOpticalGroup->getGlobalIndex())
                        //                     ->at(cHybrid->getGlobalIndex())
                        //                     ->at(cChip->getGlobalIndex())
                        //                     ->getSummary<uint16_t>();

                        auto& cClusterOccupancyC   = cClusterOccupancyH->at(cChip->getGlobalIndex());
                        auto& cClusterOccupancyCS0 = cClusterOccupancyHS0->at(cChip->getGlobalIndex());
                        auto& cClusterOccupancyCS1 = cClusterOccupancyHS1->at(cChip->getGlobalIndex());
    ...
    for(auto cOpticalGroup: *pBoard) // for on opticalGroup - begin
    {
        for(auto cHybrid: *cOpticalGroup) // for on hybrid - begin
        {
            for(auto cChip: *cHybrid) // for on chip - begin
            {
                ...
                fPedestalContainer.at(pBoard->getGlobalIndex())->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getSummary<uint16_t>() = cThreshold;
    ...
    do
    {
        ...
        for(auto cOpticalGroup: *pBoard) // for on opticalGroup - begin
        {
            for(auto cHybrid: *cOpticalGroup) // for on hybrid - begin
            {
                ...
                {
                    uint16_t cThreshold =
                        fPedestalContainer.at(pBoard->getGlobalIndex())->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getSummary<uint16_t>();
        ...
        measureBeBoardData(pBoard->getGlobalIndex(), fNevents);
        // figure out if zero was reached on all chips
        /*for(auto cOpticalGroup: *fDetectorDataContainer->at(pBoard->getGlobalIndex()))
        {
            for(auto cHybrid: *cOpticalGroup)
            {
                for(auto cChip: *cHybrid)
...
void BeamTestCheck::ScanL1Latency(uint8_t pContinousReadout)
{
    ...
    for(auto cBoard: *fDetectorContainer)
    {
        auto cTDCContainer = fTDCContainer.at(cBoard->getGlobalIndex());
    ...
    for(auto cBoard: *fDetectorContainer)
    {
        auto  cLatencyContainer      = fLatencyContainer.at(cBoard->getGlobalIndex());
        auto  cLatencyContainerS0    = fLatencyContainerS0.at(cBoard->getGlobalIndex());
        auto  cLatencyContainerS1    = fLatencyContainerS1.at(cBoard->getGlobalIndex());
        auto& cLatencyContainerCoinc = fLatencyContainerCoincidence.at(cBoard->getGlobalIndex());
        for(auto cOpticalGroup: *cBoard)
        {
            for(auto cHybrid: *cOpticalGroup)
            {
                ...
                {
                    cLatencyContainer->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->getSummary<GenericDataArray<VECSIZE, uint16_t>>()[cIndx]      = 0;
                    cLatencyContainerS0->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->getSummary<GenericDataArray<VECSIZE, uint16_t>>()[cIndx]    = 0;
                    cLatencyContainerS1->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->getSummary<GenericDataArray<VECSIZE, uint16_t>>()[cIndx]    = 0;
                    cLatencyContainerCoinc->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->getSummary<GenericDataArray<VECSIZE, uint16_t>>()[cIndx] = 0;
                }
            } // hybrid
        }     // optical group
    }
    ...
    for(auto cBoard: *fDetectorContainer)
    { cBrdTriggerMult.at(cBoard->getGlobalIndex())->getSummary<uint32_t>() = fBeBoardInterface->ReadBoardReg(cBoard, "fc7_daq_cnfg.fast_command_block.misc.trigger_multiplicity"); }
    ...
    for(auto cBoard: *fDetectorContainer)
    {
        auto& cLat      = fOptimalL1Latency.at(cBoard->getGlobalIndex());
        auto& cMaxCount = cMaximumHitCount.at(cBoard->getGlobalIndex());
        for(auto cOpticalGroup: *cBoard)
        {
            auto& cLatThisOG      = cLat->at(cOpticalGroup->getGlobalIndex());
            auto& cMaxCountThisOG = cMaxCount->at(cOpticalGroup->getGlobalIndex());
            for(auto cHybrid: *cOpticalGroup)
            {
                auto& cLatThisHybrid      = cLatThisOG->at(cHybrid->getGlobalIndex());
                auto& cMaxCountThisHybrid = cMaxCountThisOG->at(cHybrid->getGlobalIndex());
                for(auto cChip: *cHybrid)
                {
                    auto& cLatThisChip                        = cLatThisHybrid->at(cChip->getGlobalIndex());
                    auto& cMaxCountThisChip                   = cMaxCountThisHybrid->at(cChip->getGlobalIndex());
    ...
    do
    {
        ...
        for(auto cBoard: *fDetectorContainer)
        {
            auto cBrdIndx = cBoard->getGlobalIndex();
            fBeBoardInterface->setBoard(cBoard->getId());
            const std::vector<Event*>& cEvents              = this->GetEvents();
            auto&                      cTriggerMult         = cBrdTriggerMult.at(cBoard->getGlobalIndex())->getSummary<uint32_t>();
            ...
            for(size_t cTriggerId = 0; cTriggerId < cTriggerMult + 1; cTriggerId++)
            {
                ...
                for(auto cOpticalGroup: *cBoard)
                {
                    for(auto cHybrid: *cOpticalGroup)
                    {
                        auto& cHitContainerS0 = fHitOccupancyS0.at(cBrdIndx)->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex());
                        auto& cHitContainerS1 = fHitOccupancyS1.at(cBrdIndx)->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex());
                        for(auto cChip: *cHybrid)
                        {
                            auto  cLatency = fReadoutChipInterface->ReadChipReg(cChip, "TriggerLatency");
                            auto  cCrntCnt = cHitContainerS0->at(cChip->getGlobalIndex())->getSummary<uint32_t>() + cHitContainerS1->at(cChip->getGlobalIndex())->getSummary<uint32_t>();
                            auto& cMaxCnt  = cMaximumHitCount.at(cBrdIndx)->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getSummary<uint32_t>();
                            auto& cLat     = fOptimalL1Latency.at(cBrdIndx)->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getSummary<uint16_t>();
                ...
                for(auto cOpticalGroup: *cBoard)
                {
                    for(auto cHybrid: *cOpticalGroup)
                    {
                        auto& cHitContainerS0  = fHitOccupancyS0.at(cBrdIndx)->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex());
                        auto& cHitContainerS1  = fHitOccupancyS1.at(cBrdIndx)->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex());
                        auto& cCoHitCointainer = fHitOccupancyCoinc.at(cBrdIndx)->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex());
                        ...
                        for(auto cChip: *cHybrid)
                        {
                            if(cChip->getFrontEndType() == FrontEndType::MPA || cChip->getFrontEndType() == FrontEndType::MPA2 || cChip->getFrontEndType() == FrontEndType ::CBC3)
                                cIndices.push_back(cChip->getGlobalIndex());
                            else
                        ...
                        for(auto cChip: *cHybrid)
                        {
                            ...
                            {
                                auto& cHitsS0 = cHitContainerS0->at(cChip->getGlobalIndex())->getSummary<uint32_t>();
                                auto& cCoHits = cCoHitCointainer->at(cChip->getGlobalIndex())->getSummary<uint32_t>();
                                cLatencyContainerS0->at(cOpticalGroup->getGlobalIndex())
                                    ->at(cHybrid->getGlobalIndex())
                                    ->getSummary<GenericDataArray<VECSIZE, uint16_t>>()[cLatStep * (1 + cTriggerMult) - cTriggerId] += cHitsS0;
                                cLatencyContainer->at(cOpticalGroup->getGlobalIndex())
                                    ->at(cHybrid->getGlobalIndex())
                                    ->getSummary<GenericDataArray<VECSIZE, uint16_t>>()[cLatStep * (1 + cTriggerMult) - cTriggerId] += cHitsS0;
                                cLatencyContainerCoinc->at(cOpticalGroup->getGlobalIndex())
                                    ->at(cHybrid->getGlobalIndex())
                            ...
                            {
                                uint8_t cS1Index  = cChip->getGlobalIndex();
                                ...
                                cLatencyContainerS1->at(cOpticalGroup->getGlobalIndex())
                                    ->at(cHybrid->getGlobalIndex())
                ...
                for(auto cOpticalGroup: *cBoard)
                {
                    for(auto cHybrid: *cOpticalGroup)
                    {
                        auto& cS0 = cLatencyContainerS0->at(cOpticalGroup->getGlobalIndex())
                                        ->at(cHybrid->getGlobalIndex())
                                        ->getSummary<GenericDataArray<VECSIZE, uint16_t>>()[cLatStep * (1 + cTriggerMult) - cTriggerId];
                        auto& cS1 = cLatencyContainerS1->at(cOpticalGroup->getGlobalIndex())
                                        ->at(cHybrid->getGlobalIndex())
                                        ->getSummary<GenericDataArray<VECSIZE, uint16_t>>()[cLatStep * (1 + cTriggerMult) - cTriggerId];
                        auto& cM = cLatencyContainer->at(cOpticalGroup->getGlobalIndex())
                                       ->at(cHybrid->getGlobalIndex())
                                       ->getSummary<GenericDataArray<VECSIZE, uint16_t>>()[cLatStep * (1 + cTriggerMult) - cTriggerId];
                        if(cM > 0)
                        {
        ...
        for(auto cOpticalGroup: *cBoard)
        {
            for(auto cHybrid: *cOpticalGroup)
            {
                ...
                for(auto cChip: *cHybrid)
                {
                    ...
                    auto& cLat =
                        fOptimalL1Latency.at(cBoard->getGlobalIndex())->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getSummary<uint16_t>();
        ...
        for(auto cOpticalGroup: *cBoard)
        {
            for(auto cHybrid: *cOpticalGroup)
            {
                for(auto cChip: *cHybrid)
                {
                    ...
                    auto& cLyrSwap = cLyrSwp.at(cBoard->getGlobalIndex())->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getSummary<uint8_t>();
        ...
        for(auto cOpticalGroup: *cBoard)
        {
            for(auto cHybrid: *cOpticalGroup)
            {
                for(auto cChip: *cHybrid)
                {
                    ...
                    auto& cLUT =
                        cBendLUT.at(cBoard->getGlobalIndex())->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getSummary<std::vector<uint8_t>>();
                    cLUT.clear();
                    if(cChip->getFrontEndType() == FrontEndType::CBC3)
    ...
    for(auto cBoard: *fDetectorContainer)
    {
        for(auto cOpticalGroup: *cBoard)
        {
            for(auto cHybrid: *cOpticalGroup)
            {
                ...
                auto& cStbsSmry = fStubSubSet.at(cBoard->getGlobalIndex())->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->getSummary<std::vector<std::vector<uint32_t>>>();
    ...
    for(auto cBoard: *fDetectorContainer)
    {
        ...
        for(auto cOpticalGroup: *cBoard)
        {
            for(auto cHybrid: *cOpticalGroup)
            {
                ...
                {
                    fBendMap.at(cBoard->getGlobalIndex())->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->getSummary<GenericDataArray<BENDBINS, uint16_t>>()[cIndx] = 0;
                    fEventsWithSingleClusters.at(cBoard->getGlobalIndex())
                        ->at(cOpticalGroup->getGlobalIndex())
                        ->at(cHybrid->getGlobalIndex())
                        ->getSummary<GenericDataArray<BENDBINS, uint16_t>>()[cIndx]                                                                                                              = 0;
                    fEventsWithStubs.at(cBoard->getGlobalIndex())->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->getSummary<GenericDataArray<BENDBINS, uint16_t>>()[cIndx] = 0;
                }

                for(auto cChip: *cHybrid)
                {
                    cHitContainerS0->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getSummary<uint32_t>()  = 0;
                    cHitContainerS1->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getSummary<uint32_t>()  = 0;
                    cCoHitCointainer->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getSummary<uint32_t>() = 0;
                    cStubContainer->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getSummary<uint32_t>()   = 0;
                    for(uint16_t cIndx = 0; cIndx < TDCBINS; cIndx++)
                    {
                        fHitContainerTDC.at(cBoard->getGlobalIndex())
                            ->at(cOpticalGroup->getGlobalIndex())
                            ->at(cHybrid->getGlobalIndex())
                            ->at(cChip->getGlobalIndex())
                    ...
                    {
                        fHitMap.at(cBoard->getGlobalIndex())->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getChannel<Occupancy>(cIndx).fOccupancy =
                            0;
                        fStubMap.at(cBoard->getGlobalIndex())
                            ->at(cOpticalGroup->getGlobalIndex())
                            ->at(cHybrid->getGlobalIndex())
                            ->at(cChip->getGlobalIndex())
                            ->getChannel<Occupancy>(cIndx)
                            .fOccupancy                                                                                                                                                = 0;
                        cClusterContainerS0->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getChannel<Occupancy>(cIndx).fOccupancy  = 0;
                        cClusterContainerS1->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getChannel<Occupancy>(cIndx).fOccupancy  = 0;
                        cSingleStubContainer->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getChannel<Occupancy>(cIndx).fOccupancy = 0;
                    }
                } // chip
            }     // hybrid
        }         // optical group
        ...
        do
        {
            ...
            for(auto cOpticalGroup: *cBoard)
            {
                for(auto cHybrid: *cOpticalGroup)
                {
                    ...
                    auto&                cOccHybrid = fDetectorDataContainer->at(cBrdIndx)->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex());
                    std::vector<uint8_t> cIndices(0);
                    std::vector<uint8_t> cSSAIds(0);
                    for(auto cChip: *cHybrid)
                    {
                    ...
                    for(auto cChip: *cHybrid)
                    {
                        ...
                        {
                            cEventL1OccS0.at(cBoard->getGlobalIndex())
                                ->at(cOpticalGroup->getGlobalIndex())
                                ->at(cHybrid->getGlobalIndex())
                                ->at(cChip->getGlobalIndex())
                                ->getChannel<Occupancy>(cCh)
                                .fOccupancy = 0;
                            cEventL1OccS1.at(cBoard->getGlobalIndex())
                                ->at(cOpticalGroup->getGlobalIndex())
                                ->at(cHybrid->getGlobalIndex())
                                ->at(cChip->getGlobalIndex())
                                ->getChannel<Occupancy>(cCh)
                                .fOccupancy = 0;
                            cEventStubOcc.at(cBoard->getGlobalIndex())
                                ->at(cOpticalGroup->getGlobalIndex())
                                ->at(cHybrid->getGlobalIndex())
                                ->at(cChip->getGlobalIndex())
                                ->getChannel<Occupancy>(cCh)
                    ...
                    for(auto cChip: *cHybrid)
                    {
                        ...
                        auto& cLyrSwap = cLyrSwp.at(cBrdIndx)->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getSummary<uint8_t>();
                        ...
                        if(cChip->getFrontEndType() == FrontEndType::MPA || cChip->getFrontEndType() == FrontEndType::MPA2)
                        {
                            ...
                            if(cStripClusters.size() == cPxlClusters.size() && cPxlClusters.size() == 1) // events with exactly one cluster in each sensor
                            {
                                ...
                                for(auto cPxlCluster: cPxlClusters)
                                {
                                    ...
                                    cClusterContainerS0->at(cOpticalGroup->getGlobalIndex())
                                        ->at(cHybrid->getGlobalIndex())
                                        ->at(cChip->getGlobalIndex())
                                ...
                                for(auto cStrpCluster: cStripClusters)
                                {
                                    ...
                                    cClusterContainerS1->at(cOpticalGroup->getGlobalIndex())
                                        ->at(cHybrid->getGlobalIndex())
                                        ->at(cChip->getGlobalIndex())
                                        ->getChannel<Occupancy>(cRow, cCol)
                                        .fOccupancy++;
                                ...
                                for(auto cStub: cStubs)
                                {
                                    ...
                                    cSingleStubContainer->at(cOpticalGroup->getGlobalIndex())
                                        ->at(cHybrid->getGlobalIndex())
                                        ->at(cChip->getGlobalIndex())
                                        ->getChannel<Occupancy>(cRow, cCol)
                                        .fOccupancy++;
                                ...
                                if(cSingles)
                                {
                                    ...
                                    fEventsWithStubs.at(cBoard->getGlobalIndex())
                                        ->at(cOpticalGroup->getGlobalIndex())
                                        ->at(cHybrid->getGlobalIndex())
                                        ->getSummary<GenericDataArray<BENDBINS, uint16_t>>()[cDiffIndx] += cStubs.size();
                        ...
                        for(auto cStub: cStubs)
                        {
                            ...
                            fBendMap.at(cBoard->getGlobalIndex())->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->getSummary<GenericDataArray<BENDBINS, uint16_t>>()[cBendIndx]++;
                            // if(pPrint)
                            ...
                            if(cValidCoords)
                            {
                                fStubMap.at(cBrdIndx)->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getChannel<Occupancy>(cRow, cCol).fOccupancy++;
                                cEventStubOcc.at(cBrdIndx)
                                    ->at(cOpticalGroup->getGlobalIndex())
                                    ->at(cHybrid->getGlobalIndex())
                                    ->at(cChip->getGlobalIndex())
                                    ->getChannel<Occupancy>(cRow, cCol)
                        ...
                        cStubContainer->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getSummary<uint32_t>() += cStubs.size();
                        ...
                        for(auto cHit: cHits)
                        {
                            ...
                            auto& cOccChip = cOccHybrid->at(cChip->getGlobalIndex());
                            ...
                            if(cSensorID == 0)
                            {
                                cHitContainerS0->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getSummary<uint32_t>()++;
                            ...
                            {
                                cHitContainerS1->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cSSAIndex)->getSummary<uint32_t>()++;
                                // cHitContainerS1->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cSSAIndices[cIndx])->getSummary<uint32_t>()++;
                                // update hit container for each TDC phase
                                fHitContainerTDC.at(cBoard->getGlobalIndex())
                                    ->at(cOpticalGroup->getGlobalIndex())
                                    ->at(cHybrid->getGlobalIndex())
                            ...
                            {
                                cHitContainerS1->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getSummary<uint32_t>()++;
                                // update hit container for each TDC phase
                                fHitContainerTDC.at(cBoard->getGlobalIndex())
                                    ->at(cOpticalGroup->getGlobalIndex())
                                    ->at(cHybrid->getGlobalIndex())
                            ...
                            {
                                fHitMap.at(cBoard->getGlobalIndex())
                                    ->at(cOpticalGroup->getGlobalIndex())
                                    ->at(cHybrid->getGlobalIndex())
                                    ->at(cIndices[cIndx])
                                    ->getChannel<Occupancy>(cRow, cCol)
                            ...
                            else if(cValidCoords && cSSAExists)
                            {
                                ...
                                fHitMap.at(cBoard->getGlobalIndex())->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cSSAIndex)->getChannel<Occupancy>(cRow, cCol).fOccupancy++;
                                // fHitMap.at(cBoard->getGlobalIndex())->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cSSAIndices[cIndx])->getChannel<Occupancy>(cRow,
                                // cCol).fOccupancy++;
                                cTmpS1[cRow]++;
                            }
                        ...
                        cCoHitCointainer->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->at(cChip->getGlobalIndex())->getSummary<uint32_t>() += cNCoincidences;
                        cIndx++;
                    } // chip vector
                }     // hybrid vector
            }         // optical group vector
    ...
    for(auto cBoard: *fDetectorContainer)
    {
        ...
        for(auto cOpticalGroup: *cBoard)
        {
            for(auto cHybrid: *cOpticalGroup)
            {
                auto& cHitContainerS0  = fHitOccupancyS0.at(cBrdIndx)->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex());
                auto& cHitContainerS1  = fHitOccupancyS1.at(cBrdIndx)->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex());
                auto& cStubContainer   = fStubOccupancy.at(cBrdIndx)->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex());
                auto& cCoHitCointainer = fHitOccupancyCoinc.at(cBrdIndx)->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex());
                ...
                for(auto cChip: *cHybrid)
                {
                    ...
                    //     fEventsWithSingleClusters.at(cBoard->getGlobalIndex())->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->getSummary<GenericDataArray<BENDBINS,
                    //     uint16_t>>(); auto& cEventsWithStubs  =
                    //     fEventsWithStubs.at(cBoard->getGlobalIndex())->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->getSummary<GenericDataArray<BENDBINS, uint16_t>>(); float
                    ...
                    if(cChip->getFrontEndType() == FrontEndType::CBC3 && cHitContainerS0->at(cChip->getGlobalIndex())->getSummary<uint32_t>() > 0)
                        LOG(INFO) << BOLDBLUE << "Counting step...Trigger#" << +pTriggerId << " Hybrid#" << +cHybrid->getId() << " Chip#" << +(cChip->getId())
                                  << " Stubs   : " << cStubContainer->at(cChip->getGlobalIndex())->getSummary<uint32_t>() << " stubs."
                                  << " Hits S0 : " << cHitContainerS0->at(cChip->getGlobalIndex())->getSummary<uint32_t>() << " hits."
                                  << " Hits S1 : " << cHitContainerS1->at(cChip->getGlobalIndex())->getSummary<uint32_t>() << " hits." << RESET;
                    else if((cChip->getFrontEndType() == FrontEndType::MPA || cChip->getFrontEndType() == FrontEndType::MPA2) &&
                            cHitContainerS0->at(cChip->getGlobalIndex())->getSummary<uint32_t>() > 0)
                        LOG(INFO) << BOLDMAGENTA << "Counting step... Trigger#" << +pTriggerId << " Hybrid#" << +cHybrid->getId() << " Chip#" << +(cChip->getId())
                                  << " Stubs   : " << cStubContainer->at(cChip->getGlobalIndex())->getSummary<uint32_t>() << " stubs."
                                  << " Hits S0 : " << cHitContainerS0->at(cChip->getGlobalIndex())->getSummary<uint32_t>() << " hits."
                                  << " and found " << cCoHitCointainer->at(cChip->getGlobalIndex())->getSummary<uint32_t>() << " hits in the same row as S1 " << RESET;
                    else if((cChip->getFrontEndType() == FrontEndType::SSA || cChip->getFrontEndType() == FrontEndType::SSA2) &&
                            cHitContainerS1->at(cChip->getGlobalIndex())->getSummary<uint32_t>() > 0)
                    {
...
void BeamTestCheck::ScanLatency(BeBoard* pBoard, uint8_t pContinousReadout)
{
    ...
    // zero container that hold TDC information per board
    auto cTDCContainer = fTDCContainer.at(pBoard->getGlobalIndex());
    for(uint16_t cIndx = 0; cIndx < TDCBINS; cIndx++) { cTDCContainer->getSummary<GenericDataArray<VECSIZE, uint16_t>>()[cIndx] = 0; }

    // zero container
    // latency per hybrid
    auto cLatencyContainer   = fLatencyContainer.at(pBoard->getGlobalIndex());
    auto cLatencyContainerS0 = fLatencyContainerS0.at(pBoard->getGlobalIndex());
    auto cLatencyContainerS1 = fLatencyContainerS1.at(pBoard->getGlobalIndex());
    for(auto cOpticalGroup: *pBoard)
    {
        for(auto cHybrid: *cOpticalGroup)
        {
    ...
    do
    {
        ...
        uint16_t cOffset      = 0;
        auto     cBrdIndx     = pBoard->getGlobalIndex();
        size_t   cTriggerMult = fBeBoardInterface->ReadBoardReg(pBoard, "fc7_daq_cnfg.fast_command_block.misc.trigger_multiplicity");
        ...
        for(size_t cTriggerId = 0; cTriggerId < cTriggerMult + 1; cTriggerId++)
        {
            ...
            for(auto cOpticalGroup: *pBoard)
            {
                for(auto cHybrid: *cOpticalGroup)
                {
                    for(auto cChip: *cHybrid)
                    {
                        for(uint16_t cIndx = 0; cIndx < TDCBINS; cIndx++)
                        {
                            cHitContainer.at(pBoard->getGlobalIndex())
                                ->at(cOpticalGroup->getGlobalIndex())
                                ->at(cHybrid->getGlobalIndex())
                                ->at(cChip->getGlobalIndex())
                                ->getSummary<GenericDataArray<VECSIZE, uint16_t>>()[cIndx] = 0;
            ...
            do
            {
                ...
                for(auto cOpticalGroup: *pBoard)
                {
                    auto& cOccOG = cOccBrd->at(cOpticalGroup->getGlobalIndex());
                    for(auto cHybrid: *cOpticalGroup)
                    {
                        auto& cOccHybrid = cOccOG->at(cHybrid->getGlobalIndex());
                        ...
                        for(auto cChip: *cHybrid)
                        {
                            ...
                            for(auto cHit: cHits)
                            {
                                ...
                                if(cHit % 2 == 0)
                                {
                                    cLatencyContainerS0->at(cOpticalGroup->getGlobalIndex())
                                        ->at(cHybrid->getGlobalIndex())
                                        ->getSummary<GenericDataArray<VECSIZE, uint16_t>>()[cLat + cTriggerId - fStartLatency]++;
                                    cTotalHitsS0++;
                                }
                                else
                                {
...
void BeamTestCheck::ScanStubLatency(uint8_t pContinousReadout)
{
    ...
    for(auto cBoard: *fDetectorContainer)
    {
        cBrdLatency.at(cBoard->getGlobalIndex())->getSummary<uint16_t>() = 0;
        ...
        cBrdLatency.at(cBoard->getGlobalIndex())->getSummary<uint16_t>() = cModeLatency;
    ...
    for(auto cBoard: *fDetectorContainer)
    {
        auto& cLatencyContainer = fStubLatencyContainer.at(cBoard->getGlobalIndex());
        for(auto cOpticalGroup: *cBoard)
        {
            for(auto cHybrid: *cOpticalGroup)
            {
    ...
    for(auto cBoard: *fDetectorContainer)
    {
        auto& cMaxCount                                                          = cMaximumStubCount.at(cBoard->getGlobalIndex());
        cMaxCount->getSummary<uint32_t>()                                        = 0;
        fOptimalStubLatency.at(cBoard->getGlobalIndex())->getSummary<uint32_t>() = fBeBoardInterface->ReadBoardReg(cBoard, "fc7_daq_cnfg.readout_block.global.common_stubdata_delay");
    }
    ...
    do
    {
        ...
        for(auto cBoard: *fDetectorContainer)
        {
            if(cSet[cBoard->getGlobalIndex()] == 0) continue;
            fBeBoardInterface->setBoard(cBoard->getId());
            const std::vector<Event*>& cEvents              = this->GetEvents();
            auto&                      cTriggerMult         = cBrdTriggerMult.at(cBoard->getGlobalIndex())->getSummary<uint32_t>();
            float                      cNormalizationFactor = cEvents.size() / (1 + cTriggerMult);
            ...
            for(size_t cTriggerId = 0; cTriggerId < cTriggerMult + 1; cTriggerId++)
            {
                ...
                for(auto cOpticalGroup: *cBoard)
                {
                    for(auto cHybrid: *cOpticalGroup)
                    {
                        ...
                        for(auto cChip: *cHybrid)
                        {
                            auto& cStubs = cStubContainer->at(cChip->getGlobalIndex())->getSummary<uint32_t>();
                            cLatencyContainer->at(cOpticalGroup->getGlobalIndex())->at(cHybrid->getGlobalIndex())->getSummary<GenericDataArray<VECSIZE, uint16_t>>()[cLatStep + cTriggerId] += cStubs;
                            cStubCount += cStubs;
                        }
                    }
                }
    ...
    for(auto cBoard: *fDetectorContainer)
    {
        auto& cLat = fOptimalStubLatency.at(cBoard->getGlobalIndex())->getSummary<uint32_t>();
...
void BeamTestCheck::PrepareForExternalTP(BeBoard* pBoard)
{
    ...
    auto boardIndex = pBoard->getGlobalIndex();
    for(auto cOpticalGroup: *pBoard)
    {
        for(auto cHybrid: *cOpticalGroup)
        {
