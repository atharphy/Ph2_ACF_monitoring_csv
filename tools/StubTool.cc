...
void StubTool::Initialize()
{
    ...
    fBoard = static_cast<BeBoard*>(fDetectorContainer->getFirstObject());
    fCbc   = static_cast<ReadoutChip*>(fBoard->getFirstObject()->getFirstObject()->getFirstObject());
    ...
    for(uint8_t iCh = 0; iCh < nChan; iCh++)
    {
        ...
        fChannelVector.push_back(new Channel(fBoard->getId(), fBoard->getFirstObject()->getId(), fCbc->getHybridId(), fCbc->getId(), iCh));
    ...
    for(auto cBoard: *fDetectorContainer)
    {
        ...
        for(auto cOpticalGroup: *cBoard)
        {
            for(auto cHybrid: *cOpticalGroup)
            {
                ...
                for(uint8_t iCBC = 0; iCBC < nCBC; iCBC++) { configureTestPulse(static_cast<ReadoutChip*>(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC))), 1); }
                ...
                for(uint8_t tg = 0; tg < 8; tg++)
                {
                    ...
                    for(double offset = (actualbend); offset >= -(actualbend); offset -= 0.5)
                    {
                        ...
                        for(uint8_t iCBC = 0; iCBC < nCBC; iCBC++) { setCorrelationWinodwOffsets(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC)), offset, offset, offset, offset); }
                        ...
                        for(unsigned int iChan = 0; iChan < nChan; iChan++)
                        {
                            ...
                            for(uint8_t iCBC = 0; iCBC < nCBC; iCBC++)
                            {
                                ...
                                {
                                    static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC))->setReg(fChannelMaskMapCBC3[i], 0);
                                    cRegValue = static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC))->getReg(fChannelMaskMapCBC3[i]);
                                    cRegName  = fChannelMaskMapCBC3[i];
                                    fReadoutChipInterface->WriteChipReg(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC)), cRegName, cRegValue);
                                ...
                                for(unsigned int smg = 0; smg < 3; smg++)
                                {
                                    ...
                                    for(int i = 1; i <= 2; i++)
                                    {
                                        ...
                                        maskChannel(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC)), (iChan + (smg * 16) + (tg * 2) + i), false);
                                        LOG(DEBUG) << "Unmasking : " << +(iChan + (smg * 16) + (tg * 2) + i) << " = " << +iChan << " + (" << +smg << ")*16 +" << +((tg * 2) + i);
                                    }
                                }

                                // CHECKING CBC REGISTERS
                                // CheckCbcReg(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC)));
                                ...
                                for(auto cEvent: cEvents)
                                {
                                    ...
                                    if(cEvent->StubBit(cHybridId, static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC))->getId()))
                                    {
                                        uint8_t stubCounter = 1;
                                        for(auto& cStub: cEvent->StubVector(cHybridId, static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC))->getId()))
                                        {
                ...
                for(uint8_t iCBC = 0; iCBC < nCBC; iCBC++) { configureTestPulse(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC)), 0); }
            }
        }
    }
}
            ...
            for(auto cHybrid: *cOpticalGroup)
            {
                ...
                for(uint8_t tg = 0; tg < 8; tg++)
                {
                    ...
                    for(uint8_t iCBC = 0; iCBC < nCBC; iCBC++)
                    {
                        ...
                        fReadoutChipInterface->WriteChipMultReg(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC)), cRegVec);
                        cRegVec.clear();
                    }
                            ...
                            if(iChan % 96 != 0) continue;
                            {
                                for(uint8_t step = 0; step <= 1; step++)
                                {
                                    ...
                                    for(unsigned int smg = 0; smg < 3; smg++)
                                    {
                                        ...
                                        for(uint8_t iCBC = 0; iCBC < nCBC; iCBC++)
                                        {
                                            if((seedChan[smg] + bend - 1) < 0)
                                            {
                                                ...
                                                maskChannel(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC - 1)), corrChan, false);
                                                maskChannel(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC)), seedChan[smg], false);
                                                LOG(DEBUG) << "Cond 1: Corr Chan (CBC" << +(iCBC - 1) << "): " << +corrChan << " (" << +(seedChan[smg] + bend - 1) << ")";
                                                if((int)bend % 2 != 0 && corrChan + 2 > 0)
                                                {
                                                    maskChannel(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC - 1)), corrChan + 2, false);
                                            ...
                                            else if((seedChan[smg] + bend + 1) > nChan)
                                            {
                                                ...
                                                {
                                                    corrChan = nChan;
                                                    maskChannel(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC)), corrChan, false);
                                                }
                                            ...
                                            else
                                            {
                                                ...
                                                if((int)bend % 2 != 0 && corrChan + 2 > 0)
                                                {
                                                    maskChannel(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC)), corrChan + 2, false);
                                                    LOG(DEBUG) << "         , Corr Chan : " << (corrChan + 2);
                                                }
                                            }
                                            ...
                                            /*if (iCBC > 0)
                                            {
                                              maskChannel(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC-1)), 107, true);
                                              maskChannel(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC-1)), 225, true);
                                            }
                                            maskChannel(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC)), 107, true);
                                            maskChannel(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC)), 225, true);
                                            */
                                        }
                                    }
                                    ...
                                    for(auto cEvent: cEvents)
                                    {
                                        ...
                                        for(uint8_t iCBC = 0; iCBC < nCBC; iCBC++)
                                        {
                                            std::vector<uint32_t> cHits = cEvent->GetHits(cHybridId, static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC))->getId());
                                            if(cHits.size() == nChan)
                                ...
                                for(uint8_t step = 0; step <= 1; step++)
                                {
                                    ...
                                    for(unsigned int smg = 0; smg < 3; smg++)
                                    {
                                        ...
                                        for(uint8_t iCBC = 0; iCBC < nCBC; iCBC++)
                                        {
                                            ...
                                            /*if (iCBC > 0)
                                            {
                                              maskChannel(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC-1)), 108, true);
                                              maskChannel(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC-1)), 226, true);
                                            }
                                            maskChannel(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC)), 108, true);
                                            maskChannel(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC)), 226, true);
                                            */
                                        }
                                    }
                ...
                for(uint8_t regClusterWidth = 0; regClusterWidth <= 4; ++regClusterWidth)
                {
                    for(uint8_t iClusterWidth = 1; iClusterWidth <= 4; ++iClusterWidth)
                    {
                        ...
                        for(uint8_t iCBC = 0; iCBC < nCBC; iCBC++)
                        {
                            ...
                            for(int ireg = 0; ireg < 15; ireg++) { cRegVec.push_back(std::make_pair("Bend" + std::to_string(ireg), BendReg[ireg])); }
                            // fReadoutChipInterface->WriteChipMultReg (static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC)),
                            ...
                            if(teststrip + iClusterWidth > 127)
                            {
                                for(uint8_t iChan = 0; iChan < (iClusterWidth * 2); ++iChan)
                                {
                                    ...
                                    maskChannel(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC)), ((2 * teststrip) - iChan), false);
                                    ...
                                    for(uint8_t step = 0; step <= 1; step++)
                                    {
                                        ...
                                        for(unsigned int smg = 0; smg < 3; smg++)
                                        {
                                            ...
                                            for(uint8_t iCBC = 0; iCBC < nCBC; iCBC++)
                                            {
                                                ...
                                                /*if (iCBC > 0)
                                                {
                                                  maskChannel(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC-1)), 107,
                                                true); maskChannel(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC-1)),
                                                225, true);
                                                }
                                                maskChannel(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC)), 107, true);
                                                maskChannel(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC)), 225, true);
                ...
                for(uint8_t nstubs = 1; nstubs <= 5; ++nstubs)
                {
                    ...
                    for(uint8_t iCBC = 0; iCBC < nCBC; iCBC++)
                    {
                        ...
                        if(teststrip + (8 * 5) > 127)
                        {
                            for(uint8_t istub = 0; istub < nstubs; ++istub)
                            {
                                ...
                                maskChannel(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC)), (teststrip - istub * 16 + 1), false);
                                maskChannel(static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC)), (teststrip - istub * 16 + 2), false);
                            }
                        }
                        else
                        {
                    ...
                    for(auto cEvent: cEvents)
                    {
                        ...
                        for(uint8_t iCBC = 0; iCBC < nCBC; iCBC++)
                        {
                            ...
                            {
                                uint8_t stubCounter = 0;
                                uint8_t nstub       = cEvent->StubVector(cHybridId, static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC))->getId()).size();
                                for(auto& cStub: cEvent->StubVector(cHybridId, static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC))->getId()))
                                {
                                    stubCounter++;
                                    double stub_position = cStub.getPosition();
                                    ...
                                    uint16_t                     cKey  = encodeId(cHybridId, static_cast<ReadoutChip*>(cCbcVector.getObject(iCBC))->getId());
