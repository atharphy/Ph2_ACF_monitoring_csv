	    ...
	    for(auto cHybrid: *cOpticalGroup)
		{
	    	...
	    	for( size_t cAttempt=0; cAttempt < pAttempts ; cAttempt ++ ) 
	    	{
	            ...
	            do
	        	{
                	...
                	if ((cPosCopy  = cIdleBuffer.find(cCicHeader) )!= std::string::npos){
                		cIdleBuffer.erase(cPosCopy, cCicHeader.length());
                	}
				...
				LOG (INFO) << BOLDBLUE << "Attempt#" << +cAttempt << " ... idle data contains " << +cIdleBits[cIdleBits.size()-1] << " bits; balance is " << cIdleBalance[ cIdleBalance.size() -1 ] << RESET;
