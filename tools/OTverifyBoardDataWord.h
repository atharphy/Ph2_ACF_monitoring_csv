...
class OTverifyBoardDataWord : public Tool
{
  ...
  protected:
    ...
    void    runStubIntegrityTest(Ph2_HwDescription::BeBoard* theBoard, Ph2_HwInterface::D19cFWInterface* theFWInterface, uint8_t  flagCharacter = 0xea, uint8_t  idleCharacter = 0xaa); // FIXME -> remove last two arguments and move back to protected
    void    runL1IntegrityTest(Ph2_HwDescription::BeBoard* theBoard, Ph2_HwInterface::D19cFWInterface* theFWInterface, uint32_t header = 0x0ffffffe, uint32_t headerMask = 0xffffffff); // FIXME -> remove last two arguments and move back to protected
