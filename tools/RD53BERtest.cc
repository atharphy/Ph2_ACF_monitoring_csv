...
void BERtest::run()
{
    ...
    if(chain2test == 0)
        ...
        for(const auto cBoard: *fDetectorContainer)
        {
            ...
            std::vector<std::pair<uint16_t, uint16_t>> hybrid_id_chip_id;
            std::vector<uint8_t>                       chipLanes;
            for(const auto cOpticalGroup: *cBoard)
                for(const auto cHybrid: *cOpticalGroup)
                    for(const auto cChip: *cHybrid)
                    {
                        hybrid_id_chip_id.push_back(std::pair<uint16_t, uint16_t>(cHybrid->getId(), static_cast<RD53*>(cChip)->getChipLane()));
                        for(const auto lane: static_cast<RD53*>(cChip)->laneConfig.outputLaneMapping)
                            if(lane < static_cast<RD53*>(cChip)->laneConfig.nOutputLanes) chipLanes.push_back(lane);
                    }

            const auto results = fBeBoardFWMap[cBoard->getId()]->RunBERtest(given_time, frames_or_time, hybrid_id_chip_id, chipLanes, frontendSpeed);
