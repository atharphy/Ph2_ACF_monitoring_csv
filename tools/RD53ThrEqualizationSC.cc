...
void ThrEqualizationSC::localConfigure(const std::string& fileRes_, int currentRun)
{
    ...
    this->CreateResultDirectory(RD53Shared::RESULTDIR, false, false, "ThrEqualizationSC");
        ...
        for(const auto cBoard: *output)
        {
            for(const auto cOpticalGroup: *cBoard)
                for(const auto cHybrid: *cOpticalGroup)
                    for(const auto cChip: *cHybrid)
                    {
                        float theTarget = target->at(cBoard->getGlobalIndex())
                                              ->at(cOpticalGroup->getGlobalIndex())
                                              ->at(cHybrid->getGlobalIndex())
                                              ->at(cChip->getGlobalIndex())
                                              ->getSummary<ThresholdAndNoise>()
                                              .fThreshold;
