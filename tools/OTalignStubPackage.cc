...
void OTalignStubPackage::AlignStubPackageOld(BeBoard* pBoard)
{
    ...
    for(auto cOpticalGroup: *pBoard)
    {
        ...
        uint8_t hybridId = cOpticalGroup->getFirstObject()->getId();
        // uint8_t OpticalGroup = cOpticalGroup->getId();
        ...
        for(uint8_t cPackageDelay = 0; cPackageDelay < 8; cPackageDelay++)
        {
            ...
            {
                auto cIter = cBxIds.find(hybridId);
                if(cIter == cBxIds.end())
                {
                    std::vector<int> cDummy;
                    cDummy.clear();
                    cBxIds[hybridId] = cDummy;
                    cIter       = cBxIds.find(hybridId);
                }
                cIter->second.push_back(cEvent->BxId(hybridId));
                LOG(INFO) << BOLDYELLOW << "Event#" << +cEvent->GetEventCount() << "\t.. Hybrid#" << +hybridId << " BxId is " << cEvent->BxId(hybridId) << RESET;
            }
    ...
    cRegVec.push_back({"fc7_daq_cnfg.fast_command_block.user_trigger_frequency", 100});
    cRegVec.push_back({"fc7_daq_ctrl.fast_command_block.control.load_config", 0x1});
...
void OTalignStubPackage::AlignStubPackage()
{
    ...
    for(auto theBoard: *fDetectorContainer)
    {
        ...
        initialRegisterVector.push_back({"fc7_daq_cnfg.fast_command_block.user_trigger_frequency", 400});   
        initialRegisterVector.push_back({"fc7_daq_ctrl.fast_command_block.control.load_config", 0x1});
        initialRegisterVector.push_back({"fc7_daq_cnfg.fast_command_block.misc.initial_fast_reset_enable", 0});
        ...
        for(uint8_t thePackageDelay = 0; thePackageDelay < 8; thePackageDelay++)
        {
            ...
            std::cout<< __PRETTY_FUNCTION__ << " [" << __LINE__ << "] packageDelayValue = " << std::hex << packageDelayValue << std::dec << std::endl;
            ...
            for(size_t eventNumber = 0; eventNumber<numberOfEvents; ++eventNumber)
            {
                ...
                for(auto theOpticalGroup: *theBoard)
                {
                    for(auto theHybrid: *theOpticalGroup)
                    {
                        theBunchCrossingIdContainer.getHybrid(theBoard->getId(), theOpticalGroup->getId(), theHybrid->getId())->getSummary<std::vector<uint16_t>>()[eventNumber] = theEvent->BxId(theHybrid->getId());
                    }
                }
            }
            ...
            for(auto theOpticalGroup: *theBoard)
            {
                for(auto theHybrid: *theOpticalGroup)
                {
                    auto theBunchCrossingId = theBunchCrossingIdContainer.getHybrid(theBoard->getId(), theOpticalGroup->getId(), theHybrid->getId())->getSummary<std::vector<uint16_t>>();
                    std::cout << "Hybrid id = " << theHybrid->getId() << std::endl;
                    for(auto bxId: theBunchCrossingId) std::cout << bxId << " ";
                    std::cout << std::endl;
                }
            }
            ...
            //     {
            //         auto theBunchCrossingId = theBunchCrossingIdContainer.getHybrid(theBoard->getId(), theOpticalGroup->getId(), theHybrid->getId())->getSummary<std::vector<uint16_t>>();
            //         theBestPackageDelayContainer.getHybrid(theBoard->getId(), theOpticalGroup->getId(), theHybrid->getId())->getSummary<std::vector<bool>>()[thePackageDelay] = std::adjacent_find(theBunchCrossingId.begin(), theBunchCrossingId.end(), std::not_equal_to<>());
            //     }
            // }
        }
        ...
        //         if(numberOfBestPackageDelays != 0)
        //         {
        //             LOG(ERROR) << "ERROR for Board " << +theBoard->getId() << " OpticalGroup " << +theOpticalGroup->getId()<< " Hybrid " << +theHybrid->getId() << ": number of best package delay = " << numberOfBestPackageDelays << ", expected to be 1" << RESET;
        //             continue;
        //         }
        //         hybridBestPackageDelay.push_back(*std::ranges::find_if(theBestPackageDelayVector, [](bool value){return value;}))
        //     }
        //     if(hybridBestPackageDelay.size() > 0)
        //     {
        //         if(!std::adjacent_find(theBunchCrossingId.begin(), theBunchCrossingId.end(), std::not_equal_to<>()))
        //         {
        //             LOG(ERROR) << "ERROR for Board " << +theBoard->getId() << " OpticalGroup " << +theOpticalGroup->getId() <<": FW cannot handle different stub package delay within same OpticalGroup, setting the value found for Hybrid " << +theOpticalGroup->getFirstObject()->getId() << RESET;
        //         }
        //         if(cOpticalGroup->getId() < 10)
        //             bestPackageDelayLink0Link9 = (hybridBestPackageDelay[0] << cOpticalGroup->getId() % 10 * 3) | bestPackageDelayLink0Link9;
        //         else
        //             bestPackageDelayLink10Link11 = (hybridBestPackageDelay[0] << cOpticalGroup->getId() % 10 * 3) | bestPackageDelayLink10Link11;
        //     }
        // }
