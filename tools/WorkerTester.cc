...
uint8_t WorkerTester::ReadLpGBTRegister_Old(uint8_t pLinkId, uint16_t pRegisterAddress)
{
    ...
    while((cReadBackRegAddr != pRegisterAddress) && cIter < cMaxIter)
    {
        //ResetCPB();
        if(fVerbose)
            LOG(INFO) << BOLDRED << "[Iter# " << cIter << "/10 of D19cFWInterface::ReadLpGBTRegister] : Received corrupted reply from command processor block ... retrying" << RESET;
        cReplyVector.clear();
        WriteCommandCPB(cCommandVector);
        cReplyVector     = ReadReplyCPB(cExpectedReplySize);
        cReadBack        = cReplyVector[7] & 0xFF;
...
{
    if(fNewImp){
        if(fVerbose) LOG(INFO) << MAGENTA << "New WriteLpGBTRegister, Register address 0x" << std::hex << +pRegisterAddress << std::dec << " , Register value 0x" << std::hex << +pRegisterValue << std::dec << RESET;
        return WriteLpGBTRegister_New(pLinkId, pRegisterAddress, pRegisterValue);
    }
    else {
        if(fVerbose) LOG(INFO) << MAGENTA << "Old WriteLpGBTRegister, Register address 0x" << std::hex << +pRegisterAddress << std::dec << " , Register value 0x" << std::hex << +pRegisterValue << std::dec << RESET;
...
{
    if(fNewImp){
        if(fVerbose) LOG(INFO) << MAGENTA << "New I2CWrite, MasterId 0x" << std::hex << +pMasterId << std::dec << " , MasterConfig 0x" << std::hex << +pMasterConfig << std::dec << " , Slave address 0x" << std::hex << +pSlaveAddress << std::dec << " , Slave data 0x" << std::hex << +pSlaveData << std::dec << RESET;
        return I2CWrite_New(pLinkId, pMasterId, pMasterConfig, pSlaveAddress, pSlaveData);
    }
    else{
        if(fVerbose) LOG(INFO) << MAGENTA << "Old I2CWrite, MasterId 0x" << std::hex << +pMasterId << std::dec << " , MasterConfig 0x" << std::hex << +pMasterConfig << std::dec << " , Slave address 0x" << std::hex << +pSlaveAddress << std::dec << " , Slave data 0x" << std::hex << +pSlaveData << std::dec << RESET;
...
{
    if(fNewImp){
        if(fVerbose) LOG(INFO) << MAGENTA << "New I2CRead, MasterId 0x" << std::hex << +pMasterId << std::dec << " , MasterConfig 0x" << std::hex << +pMasterConfig << std::dec << " , Slave address 0x" << std::hex << +pSlaveAddress << std::dec << RESET;
        return I2CRead_New(pLinkId, pMasterId, pMasterConfig, pSlaveAddress);
    }
    else{
        if(fVerbose) LOG(INFO) << MAGENTA << "Old I2CRead, MasterId 0x" << std::hex << +pMasterId << std::dec << " , MasterConfig 0x" << std::hex << +pMasterConfig << std::dec << " , Slave address 0x" << std::hex << +pSlaveAddress << std::dec << RESET;
...
{
    if(fNewImp){
        if(fVerbose) LOG(INFO) << MAGENTA << "New WriteFERegister, Register address 0x" << std::hex << +pRegisterAddress << std::dec << " , Register Value 0x" << std::hex << +pRegisterValue << std::dec << RESET;
        return WriteFERegister_New(pChip, pRegisterAddress, pRegisterValue, pVerify);
    } 
    else{
        if(fVerbose) LOG(INFO) << MAGENTA << "Old WriteFERegister, Register address 0x" << std::hex << +pRegisterAddress << std::dec << " , Register Value 0x" << std::hex << +pRegisterValue << std::dec << RESET;
...
{           
    ...
    for(const auto& cMaster : cMasters) {
        WriteLpGBTRegister(cOpticalGroup->getId(), clpGBTRegMap["I2CM" + std::to_string(cMaster) + "Config"].fAddress, 1 << 5 | 1 << 3);
    }
}
