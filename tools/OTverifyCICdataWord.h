...
class OTverifyCICdataWord : public Tool
{
  ...
  protected:
    ...
    void                       runStubIntegrityTest2S(Ph2_HwDescription::BeBoard* theBoard, Ph2_HwInterface::D19cFWInterface* theFWInterface);
    void                       runStubIntegrityTestPS(Ph2_HwDescription::BeBoard* theBoard, Ph2_HwInterface::D19cFWInterface* theFWInterface);
    ...
    virtual PatternMatcher     injectStubsPSOld(Ph2_HwDescription::ReadoutChip* theMPA, uint8_t chipIdForCIC, uint8_t numberOfBytesInSinglePacket);
    
    virtual void               injectStubs2S(Ph2_HwDescription::ReadoutChip* theCBC, const std::vector<std::tuple<uint8_t, uint8_t, uint8_t>>& listOfStubs);
    virtual PatternMatcher     producePatternMatcher2S(uint8_t chipIdForCIC, uint8_t hybridId, const std::vector<std::tuple<uint8_t, uint8_t, uint8_t>>& listOfStubs);
    virtual void               injectStubsPS(Ph2_HwDescription::ReadoutChip* theMPA, uint8_t numberOfBytesInSinglePacket, const std::vector<std::tuple<uint8_t, uint8_t, uint8_t>>& listOfStubs);
    virtual PatternMatcher     producePatternMatcherPS(uint8_t chipIdForCIC, uint8_t numberOfBytesInSinglePacket, const std::vector<std::tuple<uint8_t, uint8_t, uint8_t>>& listOfStubs);

    GenericDataArray<float, 2> match2SstubPatterns(Ph2_HwDescription::ReadoutChip*      theChip,
    ...
    float                      matchL1Pattern(std::vector<uint32_t> theWordVector, const PatternMatcher& thePatternMatcher, uint8_t numberOfBytesInSinglePacket);
    ...
    virtual std::vector<std::vector<std::tuple<uint8_t, uint8_t, uint8_t>>> createPSstubList();
    virtual std::vector<std::vector<std::tuple<uint8_t, uint8_t, uint8_t>>> create2SstubList();
  ...
  private:
    ...
    GenericDataArray<float, 2>
         runL1Interations(Ph2_HwInterface::D19cFWInterface* theFWInterface, PatternMatcher& thePatternMatcher, Ph2_HwDescription::ReadoutChip* theChip, uint8_t numberOfBytesInSinglePacket);
