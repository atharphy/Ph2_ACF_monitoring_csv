                        ...
                        for(auto cHybrid: *cOpticalGroup)
                        {
                            ...
                            for(auto cChip: *cHybrid)
                            {
                                ...
                                theLatencyContainerS0.getObject(cBoard->getId())
                                    ->getObject(cOpticalGroup->getId())
                                    ->getObject(cHybrid->getId())
                                    ->getSummary<GenericDataArray<uint16_t, VECSIZE>>().at(cLat + cTriggerId - fStartLatency) = cTotalHitsS0;

                                theLatencyContainerS1.getObject(cBoard->getId())
                                    ->getObject(cOpticalGroup->getId())
                                    ->getObject(cHybrid->getId())
                                    ->getSummary<GenericDataArray<uint16_t, VECSIZE>>().at(cLat + cTriggerId - fStartLatency) = cTotalHitsS1;
                                theLatencyContainer.getObject(cBoard->getId())
                                    ->getObject(cOpticalGroup->getId())
                                    ->getObject(cHybrid->getId())
                                    ->getSummary<GenericDataArray<uint16_t, VECSIZE>>().at(cLat + cTriggerId - fStartLatency) = (cTotalHitsS0 + cTotalHitsS1);
                            } // chip vector
                        } // hybrid vector
                    ...
                    for(auto cOpticalGroup: *pBoard)
                    {
                        for(auto cHybrid: *cOpticalGroup)
                        {
                            ...
                            theLatencyContainer.getObject(pBoard->getId())
                                ->getObject(cOpticalGroup->getId())
                                ->getObject(cHybrid->getId())
                                ->getSummary<GenericDataArray<GenericDataArray<uint16_t, VECSIZE>, VECSIZE>>().at(cStubLatency).at((cLatency - fStartLatency)) += cNEvents_wBoth;
                        }
                    }
                ...
                for(uint16_t cLatency = fStartLatency; cLatency < fStartLatency + fLatencyRange; cLatency++)
                {
                    ...
                    for(uint8_t cStubLatency = 0; cStubLatency < cLatency; cStubLatency++)
                    {
                        uint16_t val = theLatencyContainer.getObject(pBoard->getId())
                                           ->getObject(cOpticalGroup->getId())
                                           ->getObject(cHybrid->getId())
                                           ->getSummary<GenericDataArray<GenericDataArray<uint16_t, VECSIZE>, VECSIZE>>().at(cStubLatency).at((cLatency - fStartLatency));
