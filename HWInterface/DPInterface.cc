...
namespace Ph2_HwInterface
{
  ...
  void DPInterface::ResetFCMDBram(BeBoardFWInterface* pInterface)
  {
    ...
    for( uint16_t cBx=0; cBx < 16383 ; cBx++ )
    {
      ...
      std::this_thread::sleep_for (std::chrono::microseconds (fWait_us*10) );
      if( cBx %1000 == 0 )
        LOG (INFO) << BOLDBLUE << "\t... Bx..." << +cBx << RESET;
    }
  }
  ...
  void DPInterface::GenerateDummyData()
  {
    ...
    LOG (INFO) << BOLDBLUE << "Mean trigger rate is " << +cTriggerRate 
      << " Expect to see  "
      << +cMeanNtriggers
      << " in "
      << +cNEvents
    ...
    for( size_t cIndx=0; cIndx < cNtrials; cIndx++)
    {
      ...
      LOG (INFO) << BOLDBLUE << "This batch of "
        << +cNEvents 
        << " have "
        << +cNTriggers 
        << " triggers"
      ...
      for( size_t cBx=0; cBx < cNEvents; cBx++)
      {
        ...
        else
        {
          ...
          std::string cData="";
          // header 
          for(size_t cIndx=0; cIndx < cHeaderSize; cIndx++)
          {
            cData += "1";
          }
          cData += "0";
          // two error bits 
          for(size_t cIndx=0; cIndx < cErBitsSize; cIndx++)
          {
            cData += "0";
          }
          //L1 Id 
          ...
          LOG (DEBUG) << BOLDBLUE << "Event#" << +cBx 
            << "L1 Id " << +cL1Id 
            << " generate data packet with "
            << +cNSclusters[cL1Id] << " s clusters "
            << +cNPclusters[cL1Id] << " p clusters "
            << " L1 data is " << cData
            << " length of L1 data is "
            << +cNbits
            << " expect to have "
            << (cSClstrSize)*cNSclusters[cL1Id] + (cPClstrSize)*cNPclusters[cL1Id]
            << " bits including cluster information"
            << " which will take "
            << cNbits/(8.)
            << " Bx to transmit the data "
            << " which means between "
            << +cFrstBx 
            << " and "
            << +cLstBx 
          ...
          LOG (DEBUG) << BOLDBLUE << "\t.. have "
            << +fInputCICL1Data.size() << " bytes of MPA data "
            << " and "
            << +fFastCommands.size() << " T1 cmds."
            << RESET;
          cL1Id ++;
          cL1Counter++;
        }
    ...
    LOG (INFO) << BOLDBLUE << "Mean triggers "
      << cMeanNtriggers 
      << " in "
      << +fFastCommands.size()
      << " simulated events "
  ...
  bool DPInterface::ReadL1Data(BeBoardFWInterface* pInterface)
  {
    ...
    LOG (INFO) << BOLDBLUE << "Read back data for " << +cNBxs
      << " BXs from file ... " << RESET;
    if( cNBxs < fMaxBx ) 
  ...
  uint16_t DPInterface::LoadL1Data(BeBoardFWInterface* pInterface)
  {
    ...
    size_t cNBxs= fMaxBx;
    LOG (INFO) << BOLDBLUE << "Configure CIC data player BRAM for L1 and T1 lines  for "
      << +cNBxs << " clock cycles." << RESET;
    ...
    for( uint16_t cBx=1; cBx <= cNBxs ; cBx++ )
    {
      ...
      std::sprintf (cBuffer, "fc7_daq_cnfg.physical_interface_block.mpa_to_cic_data_player_line_%01d_%01d.line%d_pattern_7_to_0"
        , cBaseIndex, cBaseIndex+1
        , cLine );
      std::string cReg0 (cBuffer);
      std::vector< std::pair<std::string, uint32_t> > cRegs;
      char cTmpBuffer[96];
      std::sprintf (cTmpBuffer, "fc7_daq_cnfg.physical_interface_block.mpa_to_cic_data_player_address_BRAM_line_%01d_%01d.line%01d_address"
        , cBaseIndex, cBaseIndex+1
        , cLine);
      ...
      LOG (DEBUG) << BOLDBLUE << "Fast command from verification framework is " << std::bitset<8>(fFastCommands[cBx-1])
        << " writing " << std::bitset<4>(cCode) 
        << " to generic fast command data player in address  " << (cBx)
        << " FE0 data " << std::bitset<8>(fInputCICL1Data[cBx-1][cFeIndx])
        << RESET;
  ...
  void DPInterface::CheckFcmdBRAM(BeBoardFWInterface* pInterface)
  {
    for( uint16_t cBx=0; cBx < fMaxBx ; cBx++ )
    {
      ...
      LOG (INFO) << BOLDBLUE << "Bx" << +cBx 
        << " BRAM contains " << std::bitset<32>(cDebug) 
        << " so write T1 " << std::bitset<4>(cData)
        << " to BRAM address " << +cAddress << RESET;
    }
  }
  uint16_t DPInterface::SendTriggers(BeBoardFWInterface* pInterface)
  {
    ...
    for( uint16_t cBx=1; cBx <= fMaxBx ; cBx++ )
    {
      ...
      pInterface->WriteReg( "fc7_daq_ctrl.fast_command_block.control.wr_en_generic", 0x1);
      auto cDebug = pInterface->ReadReg( "fc7_daq_stat.fast_command_block.generic_fcmd_debug" );
      LOG (DEBUG) << BOLDBLUE << "Bx" << +cBx << " -- fast command code " << std::bitset<4>(cCode) 
        << " BRAM debug contains " << std::bitset<32>(cDebug)
        << RESET;
  ...
  void DPInterface::ConfigureLineBRAM(BeBoardFWInterface* pInterface, uint8_t pLine, uint8_t pPattern, uint16_t pNumberOfClks)
  {
    ...
    LOG (INFO) << BOLDBLUE << "Playing... " << std::bitset<8>(pPattern) << " for "
      << +pNumberOfClks << " clock cycles." << RESET;
    ...
    {
      if( (cIndx) % 100 == 0 )
        LOG (INFO) << BOLDBLUE << "\t... writing entry#" << +cIndx << RESET;
      char cTmpBuffer[96];
      std::sprintf (cTmpBuffer, "fc7_daq_cnfg.physical_interface_block.mpa_to_cic_data_player_address_BRAM_line_%01d_%01d.line%01d_address"
        , cBaseIndex, cBaseIndex+1
        , pLine);
  ...
  void DPInterface::ConstPatternBRAMs(BeBoardFWInterface* pInterface, std::vector<uint8_t> pPatterns, uint16_t pNumberOfClks )
  {
    ...
    {
      LOG (INFO) << BOLDBLUE << "Configure CIC data player BRAM for line " << +cLine 
        << "to play... " << std::bitset<8>(cPattern) << " for "
        << +pNumberOfClks << " clock cycles." << RESET;
      //write the same pattern to all BRAMs
      // first - configure pattern 
      // on all lines 
    ...
    {
      if( (cBx) % 100 == 0 )
        LOG (INFO) << BOLDBLUE << "\t... writing entry#" << +cBx << RESET;
  ...
  void DPInterface::ConfigureLine(BeBoardFWInterface* pInterface, uint16_t pPattern, uint8_t pLine ) 
  {
    ...
    std::string cRegister0 (cBuffer); cRegister0 += "." + cReg0; 
