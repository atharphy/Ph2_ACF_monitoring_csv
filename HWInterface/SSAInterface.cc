...
uint8_t SSAInterface::ReadChipId(Chip* pChip)
{
    ...
    {
        LOG(INFO) << BOLDRED << "Failed to start e-fuse read operation from SSA on Board id " << +pChip->getBeBoardId() << " OpticalGroup id" << +pChip->getOpticalGroupId() << " Hybrid id " << +pChip->getHybridId() << " Chip id " << +pChip->getId() << " --- Hybrid will be disabled" << RESET;
