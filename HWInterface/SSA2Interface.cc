...
bool SSA2Interface::ConfigureChip(Chip* pSSA2, bool pVerifLoop, uint32_t pBlockSize)
{
    ...
    // std::vector<uint16_t> cAdressesVector = { 0x102D , 0x1008};

    // for ( auto cAddress : cAdressesVector ) 
    // {
    //     std::vector<uint32_t> cVecReq;
    //     ChipRegItem           cRegItem;
    //     cRegItem.fPage    = 0x00;
    //     cRegItem.fAddress = cAddress;
    //     cRegItem.fValue   = 0;
    //     bool cFailed;
    //     bool cRead;
    //     uint8_t cSSAId = pSSA2->getId();
    ...
    //     if(!cFailed && cRead )
    //     {
    //         LOG(INFO) << "Register at address " << +cAddress << " : " << cRegItem.fValue << RESET;
    //     }
    // }

    // this->DumpConfiguration(pSSA2, "defaultValues_SSA"+std::to_string( pSSA2->getId() )+".txt" );
...
bool SSA2Interface::WriteChipReg(Chip* pSSA2, const std::string& pRegName, uint16_t pValue, bool pVerifLoop)
{
    ...
    else if(pRegName == "TriggerLatency")
    {
        ...
        bool cReadoutMode = WriteChipSingleReg(pSSA2, "control_3", pValue & 0xFF, false);
        this->WriteChipSingleReg(pSSA2, "mask_peri_D", 0x10, pVerifLoop);
        cReadoutMode &= WriteChipSingleReg(pSSA2, "control_1", (pValue & 0x100) >> 8, false);
        return cReadoutMode;
    }
