...
bool SSA2Interface::ConfigureChip(Chip* pSSA2, bool pVerifLoop, uint32_t pBlockSize)
{
    ...
    cMap.clear();
    for(auto& cRegInMap: cSSA2RegMap) { cMap[cRegInMap.second.fAddress] = cRegInMap.second; }
    for(auto& cRegItem: cMap) { fBoardFW->EncodeReg(cRegItem.second, pSSA2->getHybridId(), pSSA2->getId(), cVec, pVerifLoop, cWrite); }
