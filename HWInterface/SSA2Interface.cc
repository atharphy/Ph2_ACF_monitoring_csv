...
//////////
	...
	bool SSA2Interface::ConfigureChip(Chip* pSSA2, bool pVerifLoop, uint32_t pBlockSize)
	{
	    ...
	    if(pVerifLoop && cSuccess)
	    {
			...
			uint16_t cIndx = 0;
			std::vector<std::string> cReadOnlyRegs{"AC_ReadCounterLSB","AC_ReadCounterMSB","ENFLAGS","DigCalibPattern_H","DigCalibPattern_L","THTRIMMING","status_reg","StripControl2","mask_strip","mask_peri_A","mask_peri_D"};
			for(auto& cRegInMap: cSSA2RegMap)
			{
			    	...
			    	&&  cRegInMap.first.find("Ring_oscillator") == std::string::npos &&  cRegInMap.first.find("ADC_out") == std::string::npos &&  cRegInMap.first.find("bist_output") == std::string::npos )
			    {
				    ...
				    {	
						...
						LOG (DEBUG) << BOLDGREEN << cRegInMap.first <<"(0x"<<std::hex << cRegInMap.second.fAddress <<")  should be 0x" << cRegInMap.second.fValue <<", is 0x"<<cRegItem.fValue<< std::dec << "(SSA2:" << (int)pSSA2->getId() <<")"<< RESET;
				}	
				cIndx++;
			}
		}
...
//////////
	uint16_t SSA2Interface::ReadChipReg(Chip* pSSA2, const std::string& pRegNode)
	{
	    ...
	    {
	    	LOG (INFO) << BOLDBLUE << "Reading back value of " << pRegNode << RESET;
...
//////////
	...
	//	uint8_t cRegValue       = (pValue << 4) | (pValue << 2) | (1 << 0);
		this->WriteChipSingleReg(pSSA2, "mask_strip", 0xff, false);
