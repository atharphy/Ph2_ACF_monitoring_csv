        ...
        for(auto cModule: *pBoard)
        {
            ...
            for(auto cHybrid: *cModule)
            {
                ...
                if(cCic != NULL)
                {
                    for(auto cChip: *cHybrid)
                    {
                        cNBytes            = (cChip->getFrontEndType() == FrontEndType::SSA2 || cChip->getFrontEndType() == FrontEndType::SSA || cChip->getFrontEndType() == FrontEndType::MPA || cChip->getFrontEndType() == FrontEndType::MPA2) ? 2 : 1;
