...
void D19cFWInterface::Start()
{
    ...
    // reset the readout before I start ..
    this->ResetReadout();
    std::this_thread::sleep_for(std::chrono::microseconds(fWait_us));
    ...
    LOG(DEBUG) << BOLDMAGENTA << "Starting triggers ..." << RESET;
    // print out config
...
void D19cFWInterface::L1ADebug(uint8_t pWait_ms, std::string& pReadLine, bool pPrintScoped, bool pUseInternalConfig)
{
    ...
    auto cWords = ReadBlockReg("fc7_daq_stat.physical_interface_block.l1a_debug", 50);
    if(pPrintScoped){ LOG(INFO) << BOLDBLUE << "Hits debug ...." << RESET; }
    std::string cBuffer = "";
    std::string cLine = "";
    for(auto cWord: cWords)
    {
        ...
        for(size_t cIndex = 0; cIndex < 4; cIndex++) { cOutputWords.push_back(cString.substr(cIndex * 8, 8)); }
        std::string cOutput = "";
        for(auto cIt = cOutputWords.end() - 1; cIt >= cOutputWords.begin(); cIt--) { cOutput += *cIt + " "; cLine += *cIt; }
        if(pPrintScoped){ LOG(INFO) << BOLDBLUE << cOutput << RESET; }
    }
...
void D19cFWInterface::ReadPSSCCountersFast(BeBoard* pBoard, std::vector<uint32_t>& pData, uint8_t pRawMode)
{
...
bool D19cFWInterface::WaitForData(BeBoard* pBoard)
{
    ...
    if(cAsync && cTriggerSource == 3)
    {
        ...
        LOG(DEBUG) << BOLDBLUE << "Going to open shutter for " << +cNevents << " ms." << RESET;
        // resync
        this->ChipReSync();
        // clear counters
        this->PS_Clear_counters(fFastCommandDuration);
