...
bool D19cFWInterface::StubTuning(const BeBoard* pBoard, bool pScope, uint8_t pNlines)
{
    ...
    for(auto cOpticalGroup: *pBoard)
    {
        ...
        for(auto cHybrid: *cOpticalGroup)
        {
            ...
            this->WriteReg("fc7_daq_cnfg.physical_interface_block.slvs_debug.hybrid_select", cHybrid->getIndex());
            this->WriteReg("fc7_daq_cnfg.physical_interface_block.slvs_debug.chip_select", 0);
...
bool D19cFWInterface::I2CWrite(uint8_t pLinkId, uint8_t pMasterId, uint8_t pSlaveAddress, uint32_t pSlaveData, uint8_t pNBytes)
{
    ...
    {
        if(fI2Cstatus != 4) LOG(DEBUG) << BOLDMAGENTA << "[D19cFWInterface::I2CWrite] Iter#" << +cIter 
                << " I2CM" << +pMasterId << " status indicates a failure 0x" << std::hex 
                << +fI2Cstatus << std::dec  
                << " transaction was to write " << +pNBytes 
                << " to slave address " << +pSlaveAddress 
                << " with data 0x" << std::hex << pSlaveData << std::dec 
                << RESET;
    ...
    if(cIter == cMaxIter)
    {
        LOG(INFO) << BOLDRED << "[D19cFWInterface::I2CWrite] Iter#" << +cIter 
                << " I2CM" << +pMasterId << " status indicates a failure 0x" << std::hex 
                << +fI2Cstatus << std::dec  
                << " transaction was to write " << +pNBytes 
                << " to slave address " << +pSlaveAddress 
