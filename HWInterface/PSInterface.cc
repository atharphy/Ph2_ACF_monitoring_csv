...
bool PSInterface::WriteChipReg(Chip* pPS, const std::string& pRegName, uint16_t pValue, bool pVerifLoop)
{
    if (pPS->getFrontEndType()== FrontEndType::MPA)
    {
      return theMPAInterface->WriteChipReg( pPS, pRegName, pValue, pVerifLoop);
    }
    else if (pPS->getFrontEndType()== FrontEndType::SSA)
    {
...
bool PSInterface::WriteChipMultReg(Chip* pPS, const std::vector<std::pair<std::string, uint16_t>>& pVecReq, bool pVerifLoop)
{
  ...
  if (pPS->getFrontEndType()== FrontEndType::MPA)
  {
    return theMPAInterface->WriteChipMultReg( pPS, pVecReq, pVerifLoop);
  }
  else if (pPS->getFrontEndType()== FrontEndType::SSA)
  {
...
bool PSInterface::WriteChipAllLocalReg(ReadoutChip* pPS, const std::string& dacName, ChipContainer& localRegValues, bool pVerifLoop)
{
  if (pPS->getFrontEndType()== FrontEndType::MPA)
  {
    return theMPAInterface->WriteChipAllLocalReg( pPS, dacName, localRegValues, pVerifLoop);
  }
  else if (pPS->getFrontEndType()== FrontEndType::SSA)
  {
...
bool PSInterface::ConfigureChip(Chip* pPS, bool pVerifLoop, uint32_t pBlockSize)
{
  ...
  if (pPS->getFrontEndType()== FrontEndType::MPA)
  {
    return theMPAInterface->ConfigureChip( pPS,  pVerifLoop,  pBlockSize);
  }
  else if (pPS->getFrontEndType()== FrontEndType::SSA)
  {
...
void PSInterface::Pix_write(ReadoutChip* cPS, ChipRegItem cRegItem, uint32_t row, uint32_t pixel, uint32_t data)
{return theMPAInterface->Pix_write( cPS, cRegItem, row, pixel, data);}

uint32_t PSInterface::Pix_read(ReadoutChip* cPS, ChipRegItem cRegItem, uint32_t row, uint32_t pixel)
{return theMPAInterface->Pix_read( cPS,  cRegItem,  row,  pixel);}



void PSInterface::Activate_async(Chip* pPS)
{return theMPAInterface->Activate_async( pPS);}

void PSInterface::Activate_sync(Chip* pPS)
...
{return theMPAInterface->Pix_Smode( pPS, p, smode);}

void PSInterface::Enable_pix_BRcal(ReadoutChip* pPS, uint32_t p, std::string polarity, std::string smode)
{return theMPAInterface->Enable_pix_BRcal( pPS,  p,  polarity,  smode);}


void PSInterface::Enable_pix_counter(ReadoutChip* pPS, uint32_t p)
{return theMPAInterface->Enable_pix_counter(pPS, p);}

void PSInterface::Enable_pix_sync(ReadoutChip* pPS, uint32_t p)
{return theMPAInterface->Enable_pix_sync(pPS, p);}


void PSInterface::Disable_pixel(ReadoutChip* pPS, uint32_t p)
{return theMPAInterface->Disable_pixel(pPS, p);}


void PSInterface::Enable_pix_digi(ReadoutChip* pPS, uint32_t p)
{return theMPAInterface->Enable_pix_digi(pPS, p);}
...
void PSInterface::Set_threshold(Chip* pPS, uint32_t th)
{
  if (pPS->getFrontEndType()== FrontEndType::MPA)
  {
    return theMPAInterface->Set_threshold( pPS,  th);
  }
  else if (pPS->getFrontEndType()== FrontEndType::SSA)
  {
...
void PSInterface::ReadASEvent(ReadoutChip* pPS, std::vector<uint32_t>& pData, std::pair<uint32_t, uint32_t> pSRange)
{
  if (pPS->getFrontEndType()== FrontEndType::MPA)
  {
    return theMPAInterface->ReadASEvent( pPS,  pData,  pSRange);
  }
  else if (pPS->getFrontEndType()== FrontEndType::SSA)
  {
