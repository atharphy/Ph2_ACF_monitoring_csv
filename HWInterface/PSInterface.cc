...
std::vector<uint8_t> PSInterface::readLUT(ReadoutChip* pPS, uint8_t pMode)
{
    ...
    if(pPS->getFrontEndType() == FrontEndType::MPA2) { cLUT = theMPA2Interface->readLUT(pPS, pMode); }
    return cLUT;
}
...
std::vector<std::pair<std::string, uint16_t>> PSInterface::ReadChipMultReg(Ph2_HwDescription::Chip* pChip, const std::vector<std::string>& theRegisterList) { return getInterface(pChip)->ReadChipMultReg(pChip, theRegisterList); }
