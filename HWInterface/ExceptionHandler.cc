...
void ExceptionHandler::initializeQueryFunctionNameContainer()
{
    ...
    ContainerFactory::copyAndInitStructure<EmptyContainer, EmptyContainer, std::vector<std::string>, std::vector<std::string>, std::vector<std::string>, std::vector<std::string>>(*fDetectorContainer, *fQueryFunctionNames);
}
...
{
    for(const auto& functionName : fQueryFunctionNames->getSummary<std::vector<std::string>,std::vector<std::string>>())
    {
        fDetectorContainer->removeQueryFunction(functionName);
    }
    for(auto board : *fDetectorContainer)
    {
        auto boardQueryFunctionNames = fQueryFunctionNames->getObject(board->getId());
        for(const auto& functionName : boardQueryFunctionNames->getSummary<std::vector<std::string>, std::vector<std::string>>())
        {
            board->removeQueryFunction(functionName);
        }

        for(auto opticalGroup : *board)
        {
            auto opticalGroupQueryFunctionNames = boardQueryFunctionNames->getObject(opticalGroup->getId());
            for(const auto& functionName : opticalGroupQueryFunctionNames->getSummary<std::vector<std::string>, std::vector<std::string>>())
            {
                opticalGroup->removeQueryFunction(functionName);
            }

            for(auto hybrid : *opticalGroup)
            {
                auto hybridQueryFunctionNames = opticalGroupQueryFunctionNames->getObject(hybrid->getId());
                for(const auto& functionName : hybridQueryFunctionNames->getSummary<std::vector<std::string>>())
                {
                    hybrid->removeQueryFunction(functionName);
                }
            }
        }
