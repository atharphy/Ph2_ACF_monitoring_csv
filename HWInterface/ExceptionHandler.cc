...
{
    if(fDetectorContainer->getObject(boardId)->getBoardType() == BoardType::D19C) { static_cast<D19cFWInterface*>(fBeBoardInterface->getFirmwareInterface(fDetectorContainer->getObject(boardId)))->EnableFrontEnds(fDetectorContainer->getObject(boardId)); }
}
