...
namespace Ph2_HwInterface
{
    ...
    void   D19cBackendAlignmentFWInterface::SetLineMode(AlignerObject pAlignerObject, LineConfiguration pLineConfiguration)
    {
        ...
        if( fVerbose == 1 ) LOG(INFO) << BOLDBLUE << "D19cBackendAlignmentFWInterface Line " << +fAlignerObject.fLine << " setting line mode to 0x" << std::hex << command_final << std::dec << RESET;
        else if( fVerbose == 2 ) LOG (DEBUG) <<  BOLDBLUE << "D19cBackendAlignmentFWInterface Line " << +fAlignerObject.fLine << " setting line mode to 0x" << std::hex << command_final << std::dec << RESET;
        WriteReg("fc7_daq_ctrl.physical_interface_block.phase_tuning_ctrl", command_final);
        std::this_thread::sleep_for(std::chrono::microseconds(fAlignerObject.fWait_us));
    }
    void   D19cBackendAlignmentFWInterface::SetLinePattern(AlignerObject pAlignerObject, LineConfiguration pLineConfiguration)
    {
        ...
        if( fVerbose == 1 ) LOG(INFO) << BOLDBLUE << "D19cBackendAlignmentFWInterface Setting line pattern size to 0x" << std::hex << command_final << std::dec << RESET;
        else if( fVerbose == 2 ) LOG(DEBUG) << BOLDBLUE << "D19cBackendAlignmentFWInterface Setting line pattern size to 0x" << std::hex << command_final << std::dec << RESET;
        WriteReg("fc7_daq_ctrl.physical_interface_block.phase_tuning_ctrl", command_final);
        // set the pattern
        ...
        if( fVerbose == 1 ) LOG(INFO) << BOLDBLUE << "D19cBackendAlignmentFWInterface Setting line pattern  to 0x" << std::hex << command_final << std::dec << RESET;
        else if( fVerbose == 2 ) LOG(DEBUG) << BOLDBLUE << "D19cBackendAlignmentFWInterface Setting line pattern  to 0x" << std::hex << command_final << std::dec << RESET;
        WriteReg("fc7_daq_ctrl.physical_interface_block.phase_tuning_ctrl", command_final);
        std::this_thread::sleep_for(std::chrono::microseconds(fAlignerObject.fWait_us));
    }
    uint8_t D19cBackendAlignmentFWInterface::GetLineStatus(AlignerObject pAlignerObject)
    {
        ...
        SetAlignerObject(pAlignerObject);
        // print header
        LOG(DEBUG) << BOLDBLUE << "D19cBackendAlignmentFWInterface \t Hybrid: " << RESET << +fAlignerObject.fHybrid << BOLDBLUE << ", Chip: " << RESET << +fAlignerObject.fChip << BOLDBLUE << ", Line: " << RESET << +fAlignerObject.fLine;
        ConfigureCommandType(0);
        uint32_t command_final = fAlignerObject.fCommand ;
        WriteReg("fc7_daq_ctrl.physical_interface_block.phase_tuning_ctrl", command_final);
        if( fVerbose == 1 ) LOG(INFO) << BOLDBLUE << "D19cBackendAlignmentFWInterface::PhaseTuner Get line status  0x" << std::hex << command_final << std::dec << RESET;
        else if( fVerbose == 2 ) LOG(INFO) << BOLDBLUE << "D19cBackendAlignmentFWInterface::PhaseTuner Get line status  0x" << std::hex << command_final << std::dec << RESET;
        ...
        WriteReg("fc7_daq_ctrl.physical_interface_block.phase_tuning_ctrl", command_final);
        if( fVerbose == 1 ) LOG(INFO) << BOLDBLUE << "D19cBackendAlignmentFWInterface::PhaseTuner Get line status  0x" << std::hex << command_final << std::dec << RESET;
        else if( fVerbose == 2 ) LOG(DEBUG) << BOLDBLUE << "D19cBackendAlignmentFWInterface::PhaseTuner Get line status  0x" << std::hex << command_final << std::dec << RESET;
        std::this_thread::sleep_for(std::chrono::microseconds(fAlignerObject.fWait_us));
        cStatus = ParseStatus();
        return cStatus;
    }
    ...
    bool D19cBackendAlignmentFWInterface::IsLineWordAligned(AlignerObject pAlignerObject)
    {
        return (fStatus.fWordAlignmentFSMstate == 14 );
    }
    bool D19cBackendAlignmentFWInterface::IsLinePhaseAligned(AlignerObject pAlignerObject)
    {
        return (fStatus.fPhaseAlignmentFSMstate == 14);
    }
