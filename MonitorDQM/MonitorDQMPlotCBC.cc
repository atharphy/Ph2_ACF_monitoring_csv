...
void MonitorDQMPlotCBC::book(TFile* theOutputFile, DetectorContainer& theDetectorStructure, const DetectorMonitorConfig& detectorMonitorConfig)
{
    ...
    for(const auto& registerName: detectorMonitorConfig.fMonitorElementList.at("CBC")) if(registerName.second) bookCBCPlots(theOutputFile, theDetectorStructure, registerName.first);
