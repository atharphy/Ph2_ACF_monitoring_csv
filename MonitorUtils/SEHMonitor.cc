...
void SEHMonitor::runMonitor()
{
    ...
    for(const auto& registerName: fDetectorMonitorConfig.fMonitorElementList.at("PowerSupply")) if(registerName.second) runPowerSupplyMonitor(registerName.first);
    for(const auto& registerName: fDetectorMonitorConfig.fMonitorElementList.at("TestCard")) if(registerName.second) runTestCardMonitor(registerName.first);
}
