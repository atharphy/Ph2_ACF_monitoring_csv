...
void FileParser::parseBeBoard(pugi::xml_node pBeBordNode, BeBoardFWMap& pBeBoardFWMap, DetectorContainer* pDetectorContainer, std::ostream& os)
{
    ...
    if(fEnableInterfaces)
    {
        ...
        else if(cBeBoard->getBoardType() == BoardType::RD53){ pBeBoardFWMap[cBeBoard->getId()] = new RD53FWInterface(cId.c_str(), cUri.c_str(), cAddressTable.c_str()); }
    }
    os << BOLDCYAN << "|"
       << "       "
       << "|"
...
//                 }
//             }
//             os << GREEN << "|\t|\t|\t|---- Threshold: Pxls 0x" << RED << std::hex << +cThresholdPxls << std::dec << " Strips 0x" <<  YELLOW << std::hex << +cThresholdStrips << std::dec << RESET <<
//             std::endl;
//         }
//     }
