...
void RegisterHelper::restoreSnapshot()
{
    ...
    for(auto theBoard: *fDetectorContainer)
    {
        ...
        {
            std::cout<< __PRETTY_FUNCTION__ << " [" << __LINE__ << "] Setting back board register " << registerNameAndValue.first << " to 0x" << std::hex << +registerNameAndValue.second << std::dec << std::endl;
        }
