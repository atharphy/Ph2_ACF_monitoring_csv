...
void RegisterHelper::dumpBeBoardRegisterIntoXml(std::string outputFileName)
{
    ...
    recursiveListNodes = [&recursiveListNodes](pugi::xml_node& theMotherNode, BeBoard* cBoard, BeBoardInterface* theBeBoardInterface, const std::string& theNodeName, const uhal::HwInterface* hwInterface, const std::string& motherNodeName)
    {
        std::string fullNodeName;
        if(motherNodeName != "")  fullNodeName = motherNodeName + "." + theNodeName;
        else fullNodeName = theNodeName;
    ...
    for(const auto cBoard: *fDetectorContainer)
    {
        ...
        if (found!=std::string::npos)
            outputFileName.erase(outputFileName.begin() + found, outputFileName.end());
        outputFileName.append(fileNameAppend);

        if (doc.save_file(outputFileName.c_str())) {
            std::cout << "XML file " << outputFileName << " created successfully." << std::endl;
        } else {
