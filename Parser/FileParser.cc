...
void FileParser::parseSettings(const std::string& pFilename, SettingsMap& pSettingsMap, std::ostream& os)
{
    std::vector<std::string> listOfStringSettings{"RegNameDAC1",
                                                  ...
                                                  "OTTimeCorrelations_UserTriggerRate",
                                                  "OTTimeCorrelations_ThresholdSigma",
                                                  "OTTimeCorrelations_NTriggersPerBurst",
                                                  "OTTimeCorrelations_DelayBetweenTriggers",
                                                  "OTTimeCorrelations_AverageFrequency",
...
void FileParser::parseRD53(pugi::xml_node theChipNode, Hybrid* cHybrid, std::string cFilePrefix, std::ostream& os, const FrontEndType& frontEndType)
{
    ...
    os << BOLDBLUE << "|\t|\t|----" << theChipNode.name() << " --> Id: " << BOLDYELLOW << chipId << BOLDBLUE << ", Lane: " << BOLDYELLOW << chipLane << BOLDBLUE << ", eFuseCode: " << BOLDYELLOW
       << eFuseCode << BOLDBLUE << ", IrefCode: " << BOLDYELLOW << IrefCode << BOLDBLUE << ", File: " << BOLDYELLOW << cFileName << BOLDBLUE << ", RxGroups: " << BOLDYELLOW << cRxGroups << BOLDBLUE
       << ", RxChannel: " << BOLDYELLOW << +cRxChannel << BOLDBLUE << ", RxPolarity: " << BOLDYELLOW << +cRxPolarity << BOLDBLUE << ", TxGroup: " << BOLDYELLOW << +cTxGroup << BOLDBLUE
       << ", TxChannel: " << BOLDYELLOW << +cTxChannel << BOLDBLUE << ", TxPolarity: " << BOLDYELLOW << +cTxPolarity << BOLDBLUE << ", Comment: " << BOLDYELLOW << cfgComment << RESET << std::endl;

    ReadoutChip* theChip;
    if(frontEndType == FrontEndType::RD53A)
        theChip = cHybrid->addChipContainer(
            chipId, new RD53A(cHybrid->getBeBoardId(), cHybrid->getFMCId(), cHybrid->getOpticalGroupId(), cHybrid->getId(), chipId, chipLane, eFuseCode, IrefCode, cFileName, cfgComment));
    else
        theChip = cHybrid->addChipContainer(
            chipId,
            new RD53B(frontEndType, cHybrid->getBeBoardId(), cHybrid->getFMCId(), cHybrid->getOpticalGroupId(), cHybrid->getId(), chipId, chipLane, eFuseCode, IrefCode, cFileName, cfgComment));
