...
void FileDumper::dumpBoardConfigurationFile(pugi::xml_node theMotherNode, BeBoard* theBoard)
{
    ...
    else if(theBoard->getEventType() == EventType::SCAS)
...
void FileDumper::dumpHybridConfigurationFile(pugi::xml_node theMotherNode, Hybrid* theHybrid)
{
    ...
    if(cCic != NULL)
    {
        ...
        std::string theFileName = "BE" + std::to_string(theHybrid->getBeBoardId()) + "_OG" + std::to_string(theHybrid->getOpticalGroupId()) + "_FE" + std::to_string(theHybrid->getId()%2) + "_CIC.txt";
        std::string theFullFileName = fOutputDirectory + theFileName;
    ...
    bool cWithMPA = (std::find_if(theHybrid->begin(), theHybrid->end(), [](Ph2_HwDescription::Chip* x) {
                         return (x->getFrontEndType() == FrontEndType::MPA) || (x->getFrontEndType() == FrontEndType::MPA2);
                     }) != theHybrid->end());
    bool cWithSSA = (std::find_if(theHybrid->begin(), theHybrid->end(), [](Ph2_HwDescription::Chip* x) {
                         return (x->getFrontEndType() == FrontEndType::SSA) || (x->getFrontEndType() == FrontEndType::SSA2);
                     }) != theHybrid->end());
...
void FileDumper::dumpChipConfigurationFile(pugi::xml_node theMotherNode, ReadoutChip* theReadoutChip)
{
    ...
    if(theReadoutChip->getFrontEndType() == FrontEndType::CBC3)
        ...
        theReadoutChipNodeName = SSA_NODE_NAME;
