...
void FileDumper::dumpChipConfigurationFile(pugi::xml_node theMotherNode, ReadoutChip* theReadoutChip)
{
    ...
    if(theReadoutChip->getFrontEndType() == FrontEndType::SSA2 || theReadoutChip->getFrontEndType() == FrontEndType::MPA2)
    {
        ...
        theReadoutChipNode.append_attribute(CHIP_TEMPERATURE_SLOPE_ATTRIBUTE_NAME)  = std::to_string(theReadoutChip->getADCSlopeCalibrationValue("TEMP_SLOPE")).c_str();
        theReadoutChipNode.append_attribute(CHIP_TEMPERATURE_OFFSET_ATTRIBUTE_NAME) = std::to_string(theReadoutChip->getADCSlopeCalibrationValue("TEMP_OFFSET")).c_str();
    }
}
