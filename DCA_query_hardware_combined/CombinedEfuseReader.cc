...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        for(auto* board: *controller.fDetectorContainer)
        {
            ...
            {
                if(optical->flpGBT == nullptr) continue;
                const uint32_t lpgbtEfuse = controller.flpGBTInterface->ReadChipFuseID(optical->flpGBT, optical->flpGBT->getVersion());
                for(auto* hybrid: *optical)
                    for(auto* chip: *hybrid)
                    {
                        rd53->ConfigureChip(chip);
                        output << board->getId() << ',' << optical->getId() << ",,0x" << std::uppercase << std::hex << std::setw(8) << std::setfill('0') << lpgbtEfuse << std::dec << ','
