...
public:
    ...
    static int nSlices() { 
      return (memoryDepth >= 9) ? 9 : (int)memoryDepth; 
    }

    static void setN(size_t n)              { LOG(INFO) << "Setting memory depth to " << n;
       memoryDepth = n; }
...
class OTTimeCorrelationDataBase: public OTTimeCorrelationConfig
{
  public:
    ...
    static CorrelationResult&     getLastFWCorrelation()   { static CorrelationResult r; return r; }
    static CorrelationResult&     getLastBWCorrelation()   { static CorrelationResult r; return r; }
    static TS3DCorrelationResult& getLastFW3DCorrelation() { static TS3DCorrelationResult r; return r; }
    static TS3DCorrelationResult& getLastBW3DCorrelation() { static TS3DCorrelationResult r; return r; }
    static std::vector<int>&      getLastFWMinHits()       { static std::vector<int> v; return v; }
    static std::vector<int>&      getLastBWMinHits()       { static std::vector<int> v; return v; }
    ...
    static void compute_min_hits()
    {
      ...
      if (mem[0].size() > getMinHits()) {
        for (size_t idx = 0; idx < S; ++idx) {
          lastFWHists.push_back(mem[idx].size());
        }
      }
      // Backward: require last event has enough hits
      if (mem[S-1].size() > getMinHits()) {
        for (size_t idx = 0; idx < S; ++idx) {
          lastBWHists.push_back(mem[S-1-idx].size());
        }
      }
      return;
    }
