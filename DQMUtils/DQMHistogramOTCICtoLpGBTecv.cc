    ...
    for(auto polarity: listOfClockPolarity)
    {
        for(auto CICStrength: listOfCICStrength)
        {
            for(auto hybridClockStrength: listOfClockStrength)
            {
                ...
                HistContainer<TH2F> ECVErrorRateHistogram(Form("CICtoLpGBT_ErrorRate_CIC_Signal_Strength_%.0f_LpGBT_Clock_Polarity_%.0f_Clock_Strength_%.0f", CICStrength, polarity, hybridClockStrength),
                                                          Form("CIC to LpGBT error rate CIC Signal Strength %.0f - LpGBT Clock Polarity %.0f Clock Strength %.0f", CICStrength, polarity, hybridClockStrength),numberOfXaxisBins,
                                                          0,
                                                          numberOfXaxisBins,
                                                          numberOfYaxisBins,
                                                          0,
