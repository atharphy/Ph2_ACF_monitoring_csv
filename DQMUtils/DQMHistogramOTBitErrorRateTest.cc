...
void DQMHistogramOTBitErrorRateTest::book(TFile* theOutputFile, DetectorContainer& theDetectorStructure, const Ph2_Parser::SettingsMap& pSettingsMap)
{
    ...
    auto setBitLabel = [numberOfLines](TAxis* theXaxis)
    {
        std::vector<std::string> hybridSide{"FEHR", "FEHL"};
        for(uint8_t hybridId = 0; hybridId < hybridSide.size(); ++hybridId)
        {
            theXaxis->SetBinLabel(1 + hybridId * numberOfLines, (std::string("L1_") + hybridSide[hybridId]).c_str());
            for(size_t stubLine = 0; stubLine < numberOfLines - 1; ++stubLine)
                theXaxis->SetBinLabel(stubLine + 2 + hybridId * numberOfLines, (std::string(Form("Stub%d_", int(stubLine))) + hybridSide[hybridId]).c_str());
        }
    };

    HistContainer<TH2F> errorRatePhaseScanHistogram("BERTerrorRatePhaseScan", "BERT error rate phase scan", numberOfLines * 2, -0.5, numberOfLines * 2 - 0.5, 64, -0.5, 63.5);
    ...
    HistContainer<TH2F> bitCounterPhaseScanHistogram("BERTtestedBitCounterPhaseScan", "BERT tested bit counter phase scan", numberOfLines * 2, -0.5, numberOfLines * 2 - 0.5, 64, -0.5, 63.5);
    ...
    bitCounterHistogram.fTheHistogram->GetYaxis()->SetTitle("Error rate");
    ...
    HistContainer<TH1I> bestPhaseHistogram("BERTbestPhase", "Bert best phase", 1, -0.5, 0.5);
...
//========================================================================================================================
void DQMHistogramOTBitErrorRateTest::fillErrorCounterPhaseScan(DetectorDataContainer& theErrorCountainer, uint16_t phaseDelay)
{
        ...
        for(auto theOpticalGroup: *theBoard)
        {
            ...
            for(auto theHybrid: *theOpticalGroup)
            {
                ...
                for(uint8_t line = 0; line < numberOfLines; ++line)
                {
                    ...
                    theBitCounterHistogram->SetBinContent(binNumber, phaseDelay+1, theErrorCounterVector.at(line).at(0));
                    theErrorCounterHistogram->SetBinContent(binNumber, phaseDelay+1, theErrorCounterVector.at(line).at(0) > 0 ? float(theErrorCounterVector.at(line).at(1)) / float(theErrorCounterVector.at(line).at(0)) : 1.);
                }
            }
        }
    }
        ...
        for(auto theOpticalGroup: *theBoard)
        {
            ...
            for(auto theHybrid: *theOpticalGroup)
            {
                ...
                for(uint8_t line = 0; line < numberOfLines; ++line)
                {
                    ...
                    theErrorCounterHistogram->SetBinContent(binNumber, theErrorCounterVector.at(line).at(0) > 0 ? float(theErrorCounterVector.at(line).at(1)) / float(theErrorCounterVector.at(line).at(0)) : 1.);
                }
            }
        }
    }
...
//========================================================================================================================
void DQMHistogramOTBitErrorRateTest::fillBERTbestPhase(DetectorDataContainer& theErrorCountainer)
{
    for(auto theBoard: theErrorCountainer)
    {
        for(auto theOpticalGroup: *theBoard)
        {
            ...
            theBestPhaseHistogram->SetBinContent(1, theOpticalGroup->getSummary<uint32_t>());
        }
    }
}
...
bool DQMHistogramOTBitErrorRateTest::fill(std::string& inputStream)
{
    ...
    if(theErrorCounterPhaseScanSerialization.attachDeserializer(inputStream))
    {
        ...
        DetectorDataContainer theDetectorData =
            theErrorCounterPhaseScanSerialization.deserializeOpticalGroupContainer<EmptyContainer, EmptyContainer, std::vector<GenericDataArray<uint64_t, 2>>, EmptyContainer>(fDetectorContainer, phase);
        fillErrorCounterPhaseScan(theDetectorData, phase);
        return true;
    }
    if(theBestPhaseSerialization.attachDeserializer(inputStream))
    {
        // std::cout << "Matched OTBitErrorRateTest FECcounter!!!!\n";
        DetectorDataContainer theDetectorData = theBestPhaseSerialization.deserializeOpticalGroupContainer<EmptyContainer, EmptyContainer, EmptyContainer, uint16_t>(fDetectorContainer);
        fillBERTbestPhase(theDetectorData);
        return true;
    }
    ...
    {
        // std::cout << "Matched OTBitErrorRateTest FECcounter!!!!\n";
        DetectorDataContainer theDetectorData = theFECcounterSerialization.deserializeOpticalGroupContainer<EmptyContainer, EmptyContainer, EmptyContainer, uint32_t>(fDetectorContainer);
