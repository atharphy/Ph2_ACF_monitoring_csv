...
{
    PowerTrimmingHistograms::fillHisto(dataContainer, theComparatorCurrentContainer);
}

void PowerTrimmingHistograms::fillPreamplifierCurrentHisto(const DetectorDataContainer& dataContainer)
{
    PowerTrimmingHistograms::fillHisto(dataContainer, thePreamplifierCurrentContainer);
}

void PowerTrimmingHistograms::fillLDACCurrentHisto(const DetectorDataContainer& dataContainer)
{
    PowerTrimmingHistograms::fillHisto(dataContainer, theLDACCurrentContainer);
}
    ...
    for(const auto cBoard: *fDetectorContainer)
        for(const auto cOpticalGroup: *cBoard)
            for(const auto cHybrid: *cOpticalGroup)
                for(const auto cChip: *cHybrid)
                {
                    auto* hAna   = hAnaInCurrContainer.getObject(cBoard->getId())->getObject(cOpticalGroup->getId())->getObject(cHybrid->getId())->getObject(cChip->getId())->getSummary<CanvasContainer<TH1F>>().fTheHistogram;
                    auto* hDig   = hDigInCurrContainer.getObject(cBoard->getId())->getObject(cOpticalGroup->getId())->getObject(cHybrid->getId())->getObject(cChip->getId())->getSummary<CanvasContainer<TH1F>>().fTheHistogram;
                    auto* hVAna  = hVINAContainer.getObject(cBoard->getId())->getObject(cOpticalGroup->getId())->getObject(cHybrid->getId())->getObject(cChip->getId())->getSummary<CanvasContainer<TH1F>>().fTheHistogram;
                    auto* hVDAna = hVDDAContainer.getObject(cBoard->getId())->getObject(cOpticalGroup->getId())->getObject(cHybrid->getId())->getObject(cChip->getId())->getSummary<CanvasContainer<TH1F>>().fTheHistogram;
                    auto* hVDig  = hVINDContainer.getObject(cBoard->getId())->getObject(cOpticalGroup->getId())->getObject(cHybrid->getId())->getObject(cChip->getId())->getSummary<CanvasContainer<TH1F>>().fTheHistogram;
                    auto* hVDDig = hVDDDContainer.getObject(cBoard->getId())->getObject(cOpticalGroup->getId())->getObject(cHybrid->getId())->getObject(cChip->getId())->getSummary<CanvasContainer<TH1F>>().fTheHistogram;
                    auto* hIrf   = hIrefContainer.getObject(cBoard->getId())->getObject(cOpticalGroup->getId())->getObject(cHybrid->getId())->getObject(cChip->getId())->getSummary<CanvasContainer<TH1F>>().fTheHistogram;
                    auto* hAnaShunt = hAnaShuntContainer.getObject(cBoard->getId())->getObject(cOpticalGroup->getId())->getObject(cHybrid->getId())->getObject(cChip->getId())->getSummary<CanvasContainer<TH1F>>().fTheHistogram;
                    auto* hDigShunt = hDigShuntContainer.getObject(cBoard->getId())->getObject(cOpticalGroup->getId())->getObject(cHybrid->getId())->getObject(cChip->getId())->getSummary<CanvasContainer<TH1F>>().fTheHistogram;

                    auto* hComp   = theComparatorCurrentContainer.getObject(cBoard->getId())->getObject(cOpticalGroup->getId())->getObject(cHybrid->getId())->getObject(cChip->getId())->getSummary<CanvasContainer<TH1F>>().fTheHistogram;
                    auto* hPreamp = thePreamplifierCurrentContainer.getObject(cBoard->getId())->getObject(cOpticalGroup->getId())->getObject(cHybrid->getId())->getObject(cChip->getId())->getSummary<CanvasContainer<TH1F>>().fTheHistogram;
                    auto* hLDAC   = theLDACCurrentContainer.getObject(cBoard->getId())->getObject(cOpticalGroup->getId())->getObject(cHybrid->getId())->getObject(cChip->getId())->getSummary<CanvasContainer<TH1F>>().fTheHistogram;

                    hAna->SetMarkerStyle(20); hAna->SetMarkerSize(0.8);
                    hDig->SetMarkerStyle(20); hDig->SetMarkerSize(0.8);
                    hVAna->SetMarkerStyle(20); hVAna->SetMarkerSize(0.8);
                    hVDAna->SetMarkerStyle(20); hVDAna->SetMarkerSize(0.8);
                    hVDig->SetMarkerStyle(20); hVDig->SetMarkerSize(0.8);
                    hVDDig->SetMarkerStyle(20); hVDDig->SetMarkerSize(0.8);
                    hIrf->SetMarkerStyle(20); hIrf->SetMarkerSize(0.8);
                    hAnaShunt->SetMarkerStyle(20); hAnaShunt->SetMarkerSize(0.8);
                    hDigShunt->SetMarkerStyle(20); hDigShunt->SetMarkerSize(0.8);
