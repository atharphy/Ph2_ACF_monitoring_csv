...
bool DQMHistogramOTSSAtoMPAecv::fill(std::string& inputStream)
{
    ...
    // As example, I'm expecting to receive a data stream from an uint32_t contained from calibration "OTSSAtoMPAecv"
    // ContainerSerialization myStreamer("OTSSAtoMPAecv");
    
    // if(myStreamer.attachDeserializer(inputStream))
    // {
    //     // It matched! Decoding data
    //     std::cout << "Matched OTSSAtoMPAecv!!!!!\n";
