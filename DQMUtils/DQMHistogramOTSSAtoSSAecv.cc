...
bool DQMHistogramOTSSAtoSSAecv::fill(std::string& inputStream)
{
    ...
    // As example, I'm expecting to receive a data stream from an uint32_t contained from calibration "OTSSAtoSSAecv"
    // ContainerSerialization myStreamer("OTSSAtoSSAecv");

    // if(myStreamer.attachDeserializer(inputStream))
    // {
    //     // It matched! Decoding data
    //     std::cout << "Matched OTSSAtoSSAecv!!!!!\n";
